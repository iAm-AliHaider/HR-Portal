...
export const useTasks = (initialFilter?: TaskFilter) => {
  ...
  const loadTasks = useCallback(async () => {
    try {
      ...
      console.log(
        "fetchTasks completed successfully, got",
        data.length,
        "tasks",
      );
      setTasks(data);
  ...
  useEffect(() => {
    const applyFilters = () => {
      ...
      if (filter.assignedTo && filter.assignedTo.length > 0) {
        result = result.filter(
          (task) =>
            task.assigned_to && filter.assignedTo!.includes(task.assigned_to),
        );
      }
      ...
      if (filter.isRecurring !== undefined) {
        result = result.filter(
          (task) => task.is_recurring === filter.isRecurring,
        );
      }
  ...
  const getTaskById = useCallback(async (taskId: string) => {
    try {
      ...
      setError(
        err instanceof Error ? err : new Error(`Failed to get task ${taskId}`),
      );
      return null;
    } finally {
      setLoading(false);
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error(`Failed to update task ${taskId}`),
        );
        return null;
      } finally {
        setLoading(false);
    ...
    async (taskId: string) => {
      try {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error(`Failed to delete task ${taskId}`),
        );
        return false;
      } finally {
        setLoading(false);
  ...
  const addComment = useCallback(
    async (taskId: string, comment: Omit<TaskComment, "id" | "created_at">) => {
      try {
        setLoading(true);
        ...
        setError(
          err instanceof Error
            ? err
            : new Error(`Failed to add comment to task ${taskId}`),
        );
        return null;
      } finally {
        setLoading(false);
  ...
  const addChecklistItem = useCallback(
    async (taskId: string, item: Omit<TaskChecklistItem, "id">) => {
      try {
        setLoading(true);
        ...
        setError(
          err instanceof Error
            ? err
            : new Error(`Failed to add checklist item to task ${taskId}`),
        );
        return null;
      } finally {
        setLoading(false);
  ...
  const updateChecklistItem = useCallback(
    async (
      ...
      try {
        setLoading(true);
        const updatedItem = await updateTaskChecklistItem(
          taskId,
          itemId,
          updates,
        );
        ...
        setError(
          err instanceof Error
            ? err
            : new Error(`Failed to update checklist item ${itemId}`),
        );
        return null;
      } finally {
        setLoading(false);
  ...
  const createCalendarEventFromTask = useCallback(
    async (taskId: string) => {
      try {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error(`Failed to create calendar event for task ${taskId}`),
        );
        return null;
      }
    },
    ...
    async (
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error("Failed to create recurring task"),
        );
        return null;
      } finally {
        setLoading(false);
    ...
    async (
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error(`Failed to create subtask for task ${parentTaskId}`),
        );
        return null;
      } finally {
        setLoading(false);
  ...
  const addTaskWatcher = useCallback(
    async (taskId: string, userId: string) => {
      try {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error(`Failed to add watcher to task ${taskId}`),
        );
        return null;
      }
    },
  ...
  const removeTaskWatcher = useCallback(
    async (taskId: string, userId: string) => {
      try {
        const task = tasks.find((t) => t.id === taskId);
        if (!task) throw new Error(`Task with ID ${taskId} not found`);
        ...
        setError(
          err instanceof Error
            ? err
            : new Error(`Failed to remove watcher from task ${taskId}`),
        );
        return null;
      }
    },
    ...
    async (taskId: string, hours: number) => {
      try {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error(`Failed to log time for task ${taskId}`),
        );
        return null;
      }
    },
    ...
    async (taskId: string) => {
      try {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error(`Failed to complete task ${taskId}`),
        );
        return null;
      }
    },
