...
interface UsePermissionsReturn {
  hasPermission: (resource: PermissionResource, action: PermissionAction, resourceId?: string) => boolean;
  checkPermission: (resource: PermissionResource, action: PermissionAction, resourceId?: string) => Promise<boolean>;
...
export function usePermissions(): UsePermissionsReturn {
  ...
  const hasAdminPermissionInDevMode = useCallback((resource: string, action: string): boolean => {
  ...
  const hasPermission = useCallback((
    ...
    const hasWildcardPermission = userPermissions.some(p => {
      ...
      return (pResource === '*' && pAction === action) ||
    ...
    return result;
  }, [user?.id, userPermissions, permissionCache, isDevelopment, hasAdminPermissionInDevMode]);
...
export function useAdminAccess(): { isAdmin: boolean; loading: boolean } {
  ...
  const isAdmin = hasPermission('system_settings', 'update') || 
