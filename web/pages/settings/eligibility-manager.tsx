...
const mockEligibilityCriteria = [
  {
    ...
    requirements: [
      ...
      { 
        id: 'prior-notice', 
        name: 'Prior Notice', 
        description: 'Request must be made at least 14 days in advance for planned leave',
        evaluationType: 'automatic',
  ...
  },
  {
    ...
    requirements: [
      ...
      { 
        ...
        description: 'Equipment must be due for replacement (older than 24 months)',
        evaluationType: 'automatic',
...
const SimpleTable = ({ children, ...props }) => <table className="w-full" {...props}>{children}</table>;
const SimpleTableHeader = ({ children, ...props }) => <thead {...props}>{children}</thead>;
const SimpleTableBody = ({ children, ...props }) => <tbody {...props}>{children}</tbody>;
const SimpleTableRow = ({ children, ...props }) => <tr className="border-b" {...props}>{children}</tr>;
const SimpleTableHead = ({ children, ...props }) => <th className="px-4 py-3 text-left text-sm font-medium text-gray-500" {...props}>{children}</th>;
const SimpleTableCell = ({ children, ...props }) => <td className="px-4 py-3 text-sm" {...props}>{children}</td>;

export default function EligibilityManager() {
  const [eligibilityCriteria, setEligibilityCriteria] = useState(mockEligibilityCriteria);
  ...
  const { isOpen: isCriteriaDialogOpen, onOpen: openCriteriaDialog, onClose: closeCriteriaDialog } = useDisclosure();
  const { isOpen: isRequirementDialogOpen, onOpen: openRequirementDialog, onClose: closeRequirementDialog } = useDisclosure();
  ...
  const handleDeleteCriteria = (id) => {
    if (confirm('Are you sure you want to delete this eligibility criteria? This may affect existing requests.')) {
  ...
  const handleSaveRequirement = () => {
    const newRequirementData = {
      ...requirementFormData,
      evaluationRule: requirementFormData.evaluationType === 'automatic' 
    ...
    if (editingRequirement !== null) {
      ...
      setFormData({ ...formData, requirements: [...formData.requirements, newRequirementData] });
    }
  ...
  return (
    <ModernDashboardLayout title="Eligibility Manager" subtitle="Configure eligibility requirements for requests">
      <div className="container mx-auto px-4 py-6 space-y-8">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Eligibility Manager</h1>
            <p className="text-gray-600">Define eligibility requirements for each request type</p>
          </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Define the requirements that employees must meet to submit requests
            </CardDescription>
          </CardHeader>
          <CardContent>
            <SimpleTable>
              ...
              <SimpleTableBody>
                {eligibilityCriteria.map((criteria) => {
                  const activeRequirements = criteria.requirements.filter(r => r.active).length;
                  
                  return (
                    <SimpleTableRow key={criteria.id}>
                      <SimpleTableCell className="font-medium">
                        {requestTypes.find(t => t.id === criteria.requestType)?.name || criteria.requestType}
                      </SimpleTableCell>
                      <SimpleTableCell>{criteria.requirements.length} requirements</SimpleTableCell>
                      <SimpleTableCell>
                        <div className="flex items-center">
                          <span className="mr-2">{activeRequirements} active</span>
                          {activeRequirements === criteria.requirements.length ? (
                            <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                              All Active
                            </Badge>
                          ) : (
                            <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200">
                              {criteria.requirements.length - activeRequirements} Inactive
                            </Badge>
                          )}
                        </div>
                      </SimpleTableCell>
                ...
                {eligibilityCriteria.length === 0 && (
                  <SimpleTableRow>
                    <SimpleTableCell colSpan={4} className="text-center py-8 text-gray-500">
                      No eligibility criteria defined. Click "New Eligibility Criteria" to create one.
                    </SimpleTableCell>
                  </SimpleTableRow>
                )}
              </SimpleTableBody>
        ...
        <Dialog isOpen={isCriteriaDialogOpen} onClose={closeCriteriaDialog}>
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              <DialogTitle>
                {isEditing ? 'Edit Eligibility Criteria' : 'Create New Eligibility Criteria'}
              </DialogTitle>
              <DialogDescription>
                {isEditing 
              ...
              <div>
                ...
                {formData.requirements.length > 0 ? (
                    ...
                    {formData.requirements.map((requirement, index) => (
                      ...
                      >
                        <div className="flex items-start">
                          <div className={`rounded-full w-6 h-6 flex items-center justify-center mr-3 mt-0.5 ${
                            requirement.active 
                              ? 'bg-green-100 text-green-800' 
                              : 'bg-gray-100 text-gray-400'
                          }`}>
                            {requirement.active ? <Check className="h-4 w-4" /> : <X className="h-4 w-4" />}
                          </div>
                          <div>
                            <div className={`font-medium ${requirement.active ? '' : 'text-gray-500'}`}>
                              {requirement.name}
                            ...
                            <div className="text-xs text-gray-500 mt-1">
                              Evaluation: {evaluationTypes.find(t => t.id === requirement.evaluationType)?.name || requirement.evaluationType}
                  ...
                  <div className="border border-dashed rounded-md p-8 text-center text-gray-500">
                    ...
                    <p className="text-sm">Click "Add Requirement" to define eligibility criteria</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <DialogFooter className="mt-6">
              <Button variant="outline" onClick={closeCriteriaDialog}>Cancel</Button>
        ...
        {/* Requirement Dialog */}
        <Dialog isOpen={isRequirementDialogOpen} onClose={closeRequirementDialog}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {editingRequirement !== null ? 'Edit Requirement' : 'Add Requirement'}
              </DialogTitle>
              <DialogDescription>
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value) => setRequirementFormData({ ...requirementFormData, evaluationType: value })}
                >
                  <SelectTrigger>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Input 
                      ...
                      onChange={(e) => setRequirementFormData({ ...requirementFormData, evaluationRule: e.target.value })}
                      placeholder="e.g., employmentDuration >= 90"
                      className="font-mono"
                    />
                  </div>
                  <p className="text-xs text-gray-500 mt-1">
                    Use variables like employmentDuration, leaveBalance, etc. in your rule expression
                  </p>
                </div>
              )}
              ...
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value) => setRequirementFormData({ ...requirementFormData, priority: value })}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="medium">Medium (Warning if not met)</SelectItem>
              ...
              <div className="flex items-center space-x-2">
                <input 
                  ...
                  onChange={(e) => setRequirementFormData({ ...requirementFormData, active: e.target.checked })}
            ...
            <DialogFooter className="mt-6">
              <Button variant="outline" onClick={closeRequirementDialog}>Cancel</Button>
