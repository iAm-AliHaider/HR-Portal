...
export default function DashboardFixed() {
  ...
  const stats = [
    ...
    { 
      ...
      value: '12', 
      icon: React.createElement(Briefcase, { className: "h-6 w-6 text-green-600" })
    },
    { 
      title: 'Pending Approvals', 
      value: '8', 
      icon: React.createElement(CalendarDays, { className: "h-6 w-6 text-yellow-600" })
    },
  ];
  ...
  const modules = [
    {
      ...
      href: '/hr-analytics',
      icon: React.createElement(BarChart3, { className: "h-8 w-8 text-blue-600" }),
    },
    {
      title: 'Recruitment',
      description: 'Manage job postings, applicants, and hiring workflows.',
      href: '/jobs',
      icon: React.createElement(Briefcase, { className: "h-8 w-8 text-green-600" }),
    },
    {
      title: 'Employee Directory',
      description: 'Browse and manage employee profiles and information.',
      href: '/people',
      icon: React.createElement(Users, { className: "h-8 w-8 text-purple-600" }),
    },
    {
      title: 'Leave Management',
      description: 'Handle employee time off requests and approvals.',
      href: '/leave',
      icon: React.createElement(CalendarDays, { className: "h-8 w-8 text-red-600" }),
    },
    {
      title: 'Performance',
      description: 'Track employee performance and conduct reviews.',
      href: '/performance',
      icon: React.createElement(Award, { className: "h-8 w-8 text-orange-600" }),
    },
    {
      title: 'Reports',
      description: 'Generate and view company reports and insights.',
      href: '/reports',
      icon: React.createElement(FileText, { className: "h-8 w-8 text-teal-600" }),
    },
  ];
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {stats.map((stat, index) => (
            <Card key={`stat-${index}`} className="shadow-sm hover:shadow-md transition-shadow">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {modules.map((item, index) => (
              <Link key={`module-${index}`} href={item.href} className="block group">
        ...
        {/* Status indicator */}
        <div style={{ padding: '10px', backgroundColor: '#d4edda', border: '1px solid #c3e6cb', borderRadius: '5px' }}>
