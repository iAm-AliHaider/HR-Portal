...
export default function SupabaseTestPage() {
  ...
  const testTables = async () => {
    const tables = ['profiles', 'employees', 'departments', 'jobs', 'applications'];
  ...
  const getTestIcon = (testName: string) => {
    if (testName.includes('Configuration')) return <Layers className="h-4 w-4" />;
    if (testName.includes('Authentication')) return <Lock className="h-4 w-4" />;
    ...
    if (testName.includes('Sign Out')) return <ShieldCheck className="h-4 w-4" />;
