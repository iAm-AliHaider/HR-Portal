...
import { Settings, Save, RotateCcw, Database, Server, Globe, Key } from 'lucide-react';
...
export default function ConfigurationPage() {
  ...
  const loadConfiguration = () => {
    ...
    [...coreConfig, ...apiConfig, ...dbConfig, ...featureConfig].forEach(item => {
      if (item.editable) {
  ...
  const formatValue = (value: string, secret: boolean = false) => {
    ...
    if (secret) return value.substring(0, 4) + '••••••••' + value.substring(value.length - 4);
    return value;
  };
                  ...
                  {section.items.map((item, itemIndex) => (
                    ...
                    >
                      <div className="col-span-4 md:col-span-3">
                        <div className="flex items-center">
                          <div>
                            <Label className="font-medium text-sm">{item.key}</Label>
                            {item.source && (
                        ...
                        {editMode && item.editable ? (
                          item.value === 'true' || item.value === 'false' ? (
                            <div className="flex items-center">
                              ...
                              />
                              <Label htmlFor={`toggle-${item.key}`} className="ml-2">
                                {editValues[item.key] === 'true' ? 'Enabled' : 'Disabled'}
                              </Label>
                            </div>
                          ) : (
                            <Input
