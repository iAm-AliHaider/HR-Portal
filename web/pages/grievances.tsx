...
const initialGrievances = [
  {
    id: 1,
    subject: "Workplace Environment",
    description: "The office temperature is too cold.",
    status: "Submitted",
    messages: [{ from: "You", text: "The office is too cold." }],
    evidence: [],
    anonymous: false,
  },
  {
    id: 2,
    subject: "Equipment",
    description: "My laptop is malfunctioning.",
    status: "Resolved",
    messages: [{ from: "HR", text: "We have replaced your laptop." }],
    evidence: ["photo.jpg"],
    anonymous: false,
  },
];
...
const Grievances = () => {
  ...
  const [selected, setSelected] = useState(
    null as null | (typeof initialGrievances)[0],
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setGrievances([
      ...grievances,
      {
        ...
        status: "Submitted",
        messages: [
          { from: anonymous ? "Anonymous" : "You", text: description },
        ],
  ...
  const sendMessage = () => {
    ...
    setGrievances((gs) =>
      gs.map((g) =>
        g.id === selected.id
          ? { ...g, messages: [...g.messages, { from: "You", text: message }] }
          : g,
      ),
    );
    setMessage("");
  ...
  const addEvidence = () => {
    ...
    setGrievances((gs) =>
      gs.map((g) =>
        g.id === selected.id
          ? { ...g, evidence: [...g.evidence, evidence] }
          : g,
      ),
    );
    setEvidence("");
        ...
        <ul className="space-y-4 mb-8">
          {grievances.map((g) => (
            ...
            >
              ...
              <span
                className={`px-2 py-1 rounded text-xs ${g.status === "Resolved" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
              >
                {g.status}
              </span>
              <button
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full relative">
              ...
              <div className="mb-2 text-xs text-gray-500">
                ...
                {selected.anonymous && (
                  <span className="ml-2 text-gray-400">(Anonymous)</span>
                )}
              </div>
              <div className="mb-2">
