...
interface EnhancedOnboardingTask {
  ...
  title: string;
  description: string;
  category: 'documentation' | 'training' | 'equipment' | 'systems' | 'introductions' | 'compliance';
  ...
  program_name?: string;
  is_overdue?: boolean;
  resources?: { name: string; url: string; type: 'document' | 'video' | 'form' | 'link' }[];
}
...
const MOCK_TASKS: EnhancedOnboardingTask[] = [
  {
    ...
    description: 'Submit required documents for employment verification with HR',
    category: 'compliance',
    priority: 'critical',
    status: 'completed',
  ...
  },
  {
    ...
    description: 'Install required development tools, IDEs, and access to repositories',
    category: 'systems',
    priority: 'high',
    status: 'completed',
    ...
    completed_by: 'emp1',
    completed_by_name: 'Sarah Williams',
    completion_notes: 'All tools installed successfully, repositories accessible',
    employee_name: 'Sarah Williams',
    department: 'Engineering',
    program_name: 'Engineering Onboarding',
    resources: [
  ...
  },
  {
    ...
    description: 'Complete mandatory cybersecurity awareness training and pass the assessment',
    category: 'training',
    priority: 'high',
    status: 'in_progress',
  ...
  },
  {
    ...
    description: 'Receive and configure laptop, monitor, and other essential equipment',
    category: 'equipment',
    priority: 'critical',
    status: 'pending',
...
const OnboardingTasksPage = () => {
  ...
  const [selectedTask, setSelectedTask] = useState<EnhancedOnboardingTask | null>(null);
  ...
  const stats = {
    ...
    overdueTasks: tasks.filter(t => t.is_overdue || (t.status !== 'completed' && new Date(t.due_date) < new Date())).length
  };
          ...
          <Container maxW="full" py={6}>
            ...
            <Flex justify="space-between" align="center" mb={6}>
              <Box>
                ...
                <Text color="gray.600">Manage and track all onboarding tasks across the organization</Text>
              </Box>
            ...
            {stats.overdueTasks > 0 && (
              <Alert status="warning" mb={6} borderRadius="lg">
                ...
                <AlertDescription>
                  {stats.overdueTasks} task{stats.overdueTasks > 1 ? 's are' : ' is'} overdue and need immediate attention.
                </AlertDescription>
              </Alert>
            )}
            ...
            <SimpleGrid columns={{ base: 2, md: 3, lg: 5 }} spacing={4} mb={6}>
              ...
              <Stat bg={cardBg} p={4} borderRadius="lg" borderWidth="1px" borderColor={borderColor}>
                ...
                <StatNumber fontSize="xl" color="green.500">{stats.completedTasks}</StatNumber>
                <StatHelpText>{completionRate}% completion</StatHelpText>
              </Stat>
              
              <Stat bg={cardBg} p={4} borderRadius="lg" borderWidth="1px" borderColor={borderColor}>
                <StatLabel fontSize="xs">In Progress</StatLabel>
                <StatNumber fontSize="xl" color="blue.500">{stats.inProgressTasks}</StatNumber>
                <StatHelpText>Currently active</StatHelpText>
              </Stat>
              
              <Stat bg={cardBg} p={4} borderRadius="lg" borderWidth="1px" borderColor={borderColor}>
                <StatLabel fontSize="xs">Pending</StatLabel>
                <StatNumber fontSize="xl" color="yellow.500">{stats.pendingTasks}</StatNumber>
                <StatHelpText>Not yet started</StatHelpText>
              </Stat>
              
              <Stat bg={cardBg} p={4} borderRadius="lg" borderWidth="1px" borderColor={borderColor}>
                <StatLabel fontSize="xs">Overdue</StatLabel>
                <StatNumber fontSize="xl" color="red.500">{stats.overdueTasks}</StatNumber>
            ...
            <Card bg={cardBg} borderColor={borderColor}>
              <CardHeader>
                <Flex justify="space-between" align="center">
                  ...
                  <Text fontSize="sm" color="gray.500">{filteredTasks.length} tasks</Text>
                </Flex>
              </CardHeader>
              <CardBody p={0}>
                <Box overflowX="auto">
                    ...
                    <Tbody>
                      {filteredTasks.map((task) => (
                        <Tr key={task.id} _hover={{ bg: useColorModeValue('gray.50', 'gray.700') }}>
                          <Td>
                            <HStack spacing={3}>
                              <Text fontSize="lg">{getCategoryIcon(task.category)}</Text>
                              <Box>
                                <Text fontWeight="medium" noOfLines={1}>{task.title}</Text>
                                <Text fontSize="sm" color="gray.500" noOfLines={1}>{task.description}</Text>
                              </Box>
                            </HStack>
                          </Td>
                          <Td>
                            <Text fontWeight="medium">{task.employee_name}</Text>
                            <Text fontSize="sm" color="gray.500">{task.department}</Text>
                          </Td>
                          <Td>
                            <Tag size="sm" colorScheme="blue" variant="subtle">
                              <TagLabel textTransform="capitalize">{task.category}</TagLabel>
                            </Tag>
                          </Td>
                          <Td>
                          ...
                          <Td>
                            ...
                            {(task.is_overdue || (task.status !== 'completed' && new Date(task.due_date) < new Date())) && (
                              <Badge colorScheme="red" variant="solid" ml={1} fontSize="xs">
                                Overdue
                              </Badge>
                            )}
                          </Td>
                          <Td>
                            ...
                            <Text fontSize="xs" color="gray.500">{formatRelativeDate(task.due_date)}</Text>
                          </Td>
                          <Td>
                            <Menu>
                              <MenuButton as={IconButton} icon={<SettingsIcon />} variant="ghost" size="sm" />
                              <MenuList>
                                <MenuItem icon={<ViewIcon />} onClick={() => handleViewTask(task)}>
                                  View Details
                                </MenuItem>
                                {task.status !== 'completed' && (
                                  <MenuItem icon={<CheckCircleIcon />} onClick={() => handleMarkComplete(task.id)}>
                                    Mark Complete
                                  </MenuItem>
                                )}
                ...
                <VStack spacing={6} align="stretch">
                  <Grid templateColumns={{ base: "1fr", md: "repeat(3, 1fr)" }} gap={4}>
                    <Stat textAlign="center">
                      ...
                      <Badge colorScheme={getStatusColor(selectedTask.status)} variant="subtle">
                        {selectedTask.status.replace('_', ' ')}
                      </Badge>
                    </Stat>
                    <Stat textAlign="center">
                      <StatLabel>Priority</StatLabel>
                      <Badge colorScheme={getPriorityColor(selectedTask.priority)} variant="subtle">
                        {selectedTask.priority}
                      </Badge>
                    </Stat>
                    <Stat textAlign="center">
                      <StatLabel>Duration</StatLabel>
                      <StatNumber fontSize="lg">{selectedTask.estimated_duration}m</StatNumber>
                    </Stat>
                  </Grid>
                  ...
                  <Grid templateColumns={{ base: "1fr", md: "repeat(2, 1fr)" }} gap={6}>
                    <Box>
                      <Text fontWeight="medium" mb={2}>Task Details</Text>
                      <VStack spacing={2} align="stretch">
                        <Flex justify="space-between">
                          <Text fontSize="sm" color="gray.500">Employee:</Text>
                          <Text fontSize="sm">{selectedTask.employee_name}</Text>
                        </Flex>
                        <Flex justify="space-between">
                        ...
                        <Flex justify="space-between">
                          ...
                          <Text fontSize="sm">{selectedTask.assigned_to_name || 'Unassigned'}</Text>
                        </Flex>
                      </VStack>
                    </Box>
                    ...
                    {selectedTask.resources && selectedTask.resources.length > 0 && (
                      <Box>
                        <Text fontWeight="medium" mb={2}>Resources</Text>
                        <VStack spacing={2} align="stretch">
                          {selectedTask.resources.map((resource, index) => (
                            <Flex key={index} align="center" p={2} borderRadius="md" bg={useColorModeValue('gray.50', 'gray.700')}>
                              <AttachmentIcon mr={2} />
                  ...
                  {selectedTask.completion_notes && (
                    <Box>
                      <Text fontWeight="medium" mb={2}>Completion Notes</Text>
                      <Text fontSize="sm" color="gray.600">{selectedTask.completion_notes}</Text>
                    </Box>
                  )}
                </VStack>
              )}
