...
const EmployeeReportsPage = () => {
  ...
  const [reportType, setReportType] = useState<
    "department" | "location" | "hiring" | "tenure"
  >("department");
  ...
  const trendsColumns: Column<HiringTrend>[] = [
    ...
    { key: "count", header: "New Hires", accessor: "count" },
    {
      ...
      accessor: "departments",
      cell: (departments) => {
        const sortedDepts = Object.entries(
          departments as Record<string, number>,
        )
          .sort(
            ([, aCount], [, bCount]) => (bCount as number) - (aCount as number),
          )
          .slice(0, 2);
  ...
  const renderReport = () => {
    switch (reportType) {
      case "department":
        return (
          <DataTable columns={departmentColumns} data={departmentMetrics} />
        );
      case "location":
        return <DataTable columns={locationColumns} data={locationMetrics} />;
      case "hiring":
        return <DataTable columns={trendsColumns} data={hiringTrends} />;
      case "tenure":
        return <DataTable columns={tenureColumns} data={tenureData} />;
      default:
        return null;
    }
  };
          ...
          <CardHeader>
            <div className="flex justify-between items-center">
              <div>
                ...
                <CardDescription>
                  View detailed reports and metrics about your organization
                </CardDescription>
              </div>
