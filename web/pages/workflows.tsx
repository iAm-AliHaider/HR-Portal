...
const WorkflowManagementPage = () => {
  ...
  // API hooks
  const { workflows, loading, error, createWorkflow, startWorkflow } =
    useWorkflows();

  // UI state
  const [selectedWorkflow, setSelectedWorkflow] = useState<any>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [startWorkflowData, setStartWorkflowData] = useState({
    title: "",
    assignee_email: "",
  });
  ...
  const { searchTerm, setSearchTerm, filteredItems } = useSearch(workflows, [
    "name",
    "category",
    "description",
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const stepsArray = form.values.steps
        .split("\n")
        .filter((step) => step.trim());
      const approversArray = form.values.approvers
        .split(",")
  ...
  const handleStartWorkflow = async () => {
    if (
      !selectedWorkflow ||
      !startWorkflowData.title ||
      !startWorkflowData.assignee_email
    )
      return;
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Create and manage business process workflows
              </p>
            </div>
            <button
        ...
        {totalPages > 1 && (
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-700">
              ...
              {Math.min(currentPage * 9, filteredItems.length)} of{" "}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          form.setValue("trigger_type", e.target.value)
                        }
                        className="w-full border border-gray-300 rounded-md px-3 py-2"
                        required
                      >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          form.setValue("approvers", e.target.value)
                        }
                        className="w-full border border-gray-300 rounded-md px-3 py-2"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      ...
                      <p>
                        <span className="font-medium">Deadline:</span>{" "}
                        {selectedWorkflow.deadline_days} days
                      </p>
                      <p>
                  ...
                  selectedWorkflow.approvers.length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedWorkflow.approvers.map(
                          (approver: string, index: number) => (
                            <span
                              key={index}
