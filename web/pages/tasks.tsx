...
const TaskDependencies = ({ task, getTaskById, getSubtasks }) => {
  ...
  return (
    <div className="mt-4">
      <h4 className="text-sm font-medium text-gray-700 mb-2">Task Dependencies</h4>
      {parentTask && (
        <div className="mb-2">
...
const RecurrencePattern = ({ task, createRecurringTask }) => {
  ...
  const handleCreateNextOccurrence = async () => {
    ...
    if (task.recurrence_end_date && isAfter(nextDueDate, parseISO(task.recurrence_end_date))) {
  ...
  if (task.status === 'Completed' && task.is_recurring) {
    return (
      <div className="mt-4 p-3 bg-blue-50 rounded border border-blue-100">
        <h4 className="text-sm font-medium text-blue-800 mb-1">Recurring Task</h4>
...
const TasksList = () => {
  ...
  const renderTaskBoard = () => (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
      {(['Todo', 'In Progress', 'On Hold', 'Completed'] as Task['status'][]).map(status => (
        <div key={status} className="bg-gray-50 rounded-lg p-4">
          <h3 className="font-medium text-gray-700 mb-4 flex justify-between items-center">
            {status} <span className="text-gray-500 text-sm">{groupedTasks[status].length}</span>
          </h3>
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4 mb-4">
        ...
        <div className="flex items-center space-x-2">
          <select
            ...
            onChange={e => setFilter(prev => ({ ...prev, showCompleted: e.target.value === 'all' }))}
          >
          ...
          <select
            className="px-3 py-2 border border-gray-300 rounded-md text-sm"
            value={filter.priority && filter.priority.length === 1 ? filter.priority[0] : ''}
            onChange={e => {
  ...
  const renderTaskDetail = () => {
    ...
    const taskIsOverdue = isOverdue(selectedTask.due_date) && selectedTask.status !== 'Completed';
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <input
                type="checkbox"
                            ...
                            onChange={e => handleToggleChecklistItem(selectedTask.id, item.id, e.target.checked)}
                            className="mt-1 mr-2"
                          />
                          <div className="flex-1">
                            <p className={`${item.completed ? 'line-through text-gray-400' : 'text-gray-700'}`}>
                              {item.content}
                            </p>
                    ...
                    <p className="text-gray-500 italic">No checklist items yet.</p>
                  )}
                  ...
                  {selectedTask.comments && selectedTask.comments.length > 0 ? (
                    <div className="space-y-3">
                      {selectedTask.comments.map(comment => {
                        const user = users.find(u => u.id === comment.user_id) || { full_name: 'Unknown User' };
                        return (
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="bg-gray-50 p-4 rounded space-y-3">
                    <div>
                      ...
                      <div className={`text-sm ${taskIsOverdue ? 'text-red-600 font-medium' : 'text-gray-900'}`}>
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-900">{selectedTask.category || 'Uncategorized'}</div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-500">Assigned To</div>
                      <div className="text-sm text-gray-900">
                        {users.find(u => u.id === selectedTask.assigned_to)?.full_name || 'Unassigned'}
                      </div>
                    </div>
                    <div>
                ...
                <div className="space-y-2">
                  ...
                  {selectedTask.status !== 'Completed' ? (
                    ...
                    <button
                      onClick={() => editTask(selectedTask.id, { status: 'Todo', completed_at: undefined })}
            ...
            <form onSubmit={handleAddTask}>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      className="w-full border rounded px-3 py-2 text-sm"
                      value={newTask.due_date ? format(new Date(newTask.due_date), 'yyyy-MM-dd') : ''}
                      onChange={e => setNewTask({ ...newTask, due_date: e.target.value ? new Date(e.target.value).toISOString() : '' })}
                      required
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={e => setNewTask({ ...newTask, team_id: e.target.value || undefined })}
                    >
                      <option value="">None</option>
                      {teams.map(team => (
                        <option key={team.id} value={team.id}>{team.name}</option>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={e => setNewTask({ ...newTask, assigned_to: e.target.value || undefined })}
                    >
                ...
                <div className="flex items-center">
                  ...
                  />
                  <label htmlFor="isRecurring" className="text-sm text-gray-700">Recurring Task</label>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        className="w-full border rounded px-3 py-2 text-sm"
                        value={newTask.recurrence_end_date ? format(new Date(newTask.recurrence_end_date), 'yyyy-MM-dd') : ''}
                        onChange={e => setNewTask({ 
                          ...newTask, 
                          recurrence_end_date: e.target.value ? new Date(e.target.value).toISOString() : undefined 
                        })}
                      />
                    </div>
                  </div>
      ...
      {loading ? (
        ...
        <div className="bg-red-50 border border-red-200 text-red-700 p-6 rounded-md flex flex-col items-center">
          ...
          <p className="mt-4 text-sm text-red-600">
            If the error persists, please check your network connection and make sure the API is accessible.
          </p>
        </div>
      ) : (
        <>
