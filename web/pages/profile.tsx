      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-xl">
        <div className="flex flex-col items-center mb-6">
          <Image
            src={profile.avatar}
            alt={profile.name}
            className="w-20 h-20 rounded-full mb-2 border-2 border-blue-100 object-cover"
            width={80}
            height={80}
          />
        ...
        {tab === "Security" && (
          <form onSubmit={handlePwSubmit} className="space-y-4">
            ...
            {pwMsg && (
              <div className="text-center text-sm text-blue-600">{pwMsg}</div>
            )}
            <button
        ...
        {tab === "Activity" && (
          <ul className="space-y-2">
            {mockActivity.map((act) => (
              <li
                key={act.id}
                className="flex items-center gap-4 bg-gray-50 rounded p-3 text-sm"
              >
