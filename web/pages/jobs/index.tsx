...
export default function JobsPage() {
  ...
  const { searchTerm, setSearchTerm, filteredItems } = useSearch(jobs, [
    "title",
    "department",
    "location",
    "type",
  ...
  const applyFilters = (filters: JobFilterValues) => {
          ...
          (dept) =>
            ...
            dept ===
              departmentOptions.find(
                (d) => d.label.toLowerCase() === job.department.toLowerCase(),
              )?.id,
        );
      });
    }
    ...
    if (filters.types.length > 0) {
      results = results.filter((job) => {
        ...
        return filters.types.some(
          (type) =>
            jobType === type ||
            type ===
              typeOptions.find(
                (t) => t.label.toLowerCase() === job.type.toLowerCase(),
              )?.id,
        );
      });
    }
  ...
  const handleCloseJob = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to close job",
      );
    } finally {
      setIsSubmitting(false);
    }
          ...
          <div className="text-center max-w-md mx-auto">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
              ...
              <p className="text-yellow-700 mb-4">
                We're having trouble connecting to our job database. This might
