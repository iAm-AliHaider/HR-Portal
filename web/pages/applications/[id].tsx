...
const ApplicationDetailPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [candidateData, jobData, interviewsData, assessmentsData] =
          await Promise.all([
        ...
        if (jobData && jobData.stage_ids) {
          ...
          const stagePromises = jobData.stage_ids.map((stageId) =>
            getJobStageById(stageId),
          );
          stageObjects = (await Promise.all(stagePromises)).filter(
            Boolean,
          ) as JobStage[];
  ...
  const handleInterviewScheduled = async (interview: Partial<Interview>) => {
    try {
      ...
      const updatedInterviews = await getInterviewsByApplicationId(
        application!.id,
      );
  ...
  return (
    <>
      <Head>
        <title>
          {application.candidate?.full_name || "Candidate"} - Application
          Details
        </title>
      </Head>
              ...
              <CardBody>
                <Flex justify="space-between" align="center" mb={4}>
                  ...
                  <Text fontSize="sm" color="gray.500">
                    {application.currentStage?.name || "Unknown Stage"} •{" "}
                        ...
                        <CardBody>
                          <SimpleGrid columns={2} spacing={4}>
                            ...
                            <Box>
                              ...
                              <Text fontWeight="medium">
                                {formatDate(
                                  application.last_activity_date ||
                            ...
                            {application.salary_expectation && (
                              <>
                                <Box>
                                  ...
                                  <Text fontWeight="medium">
                                    $
                                    {application.salary_expectation.amount?.toLocaleString()}{" "}
                                    {application.salary_expectation.currency}/
                                    {application.salary_expectation.period}
                                  </Text>
                                </Box>
                                <Box>
                                  <Text fontSize="sm" color="gray.500" mb={1}>
                        ...
                        >
                          <VStack align="stretch" spacing={4}>
                            ...
                            <Box>
                              ...
                              <Text fontWeight="medium">
                                {formatDate(application.candidate.created_at)}
                              </Text>
                            </Box>
                          </VStack>
                    ...
                    <CardBody>
                      {interviews.length === 0 ? (
                        <Alert status="info">
                          ...
                          <AlertDescription>
                            Schedule an interview to begin the interview
                            process.
                          </AlertDescription>
                        </Alert>
                      ) : (
                    ...
                    <CardBody>
                      {assessments.length === 0 ? (
                        <Alert status="info">
                          ...
                          <AlertDescription>
                            Send technical or skills assessments to evaluate the
                            candidate.
                          </AlertDescription>
                        </Alert>
                      ) : (
        ...
        <Modal isOpen={isScoreOpen} onClose={onScoreClose}>
          ...
          <ModalContent>
            ...
            <ModalFooter>
              ...
              <Button colorScheme="blue">Update Score</Button>
            </ModalFooter>
          </ModalContent>
        </Modal>
      </ModernDashboardLayout>
