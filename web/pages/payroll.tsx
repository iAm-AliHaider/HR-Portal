...
const PayrollManagement = () => {
  ...
  const payrollMetrics = [
    {
      label: "Total Payroll",
      value: payrollData.analytics?.current_period?.total_gross
        ? `$${payrollData.analytics.current_period.total_gross.toLocaleString()}`
        : "$485,600",
      icon: "💰",
    ...
    },
    {
      ...
      value:
        payrollData.analytics?.current_period?.total_employees?.toString() ||
        "247",
      icon: "👥",
      color: "text-blue-600",
      trend: "stable",
    },
    {
      ...
      value: payrollData.analytics?.current_period?.processing_status
        ? `${payrollData.analytics.current_period.processing_status}%`
        : "98%",
      icon: "⚡",
    ...
    {
      label: "Net Payroll",
      value: payrollData.analytics?.current_period?.total_net
        ? `$${payrollData.analytics.current_period.total_net.toLocaleString()}`
        : "$364,200",
      icon: "🏛️",
    ...
    payrollData.employees.length > 0
      ? payrollData.employees.map((emp) => ({
          ...
          deductions: {
            ...
            medicare: 119.63,
            health_insurance: emp.deductions?.health_insurance?.amount || 285.0,
            dental_insurance: emp.deductions?.dental_insurance?.amount || 45.0,
            retirement_401k: 825.05,
            life_insurance: emp.deductions?.life_insurance?.amount || 12.0,
          },
          status: emp.status,
        }))
      : [];
            ...
            <div className="flex justify-between items-center">
              <div>
                <h3 className="font-medium">
                  Pay Period: June 1 - June 30, 2024
                </h3>
                <p className="text-sm text-gray-600">Pay Date: July 5, 2024</p>
              </div>
              <span
                className={`px-3 py-1 rounded-full text-sm ${getStatusColor("processing")}`}
              >
                PROCESSING
              </span>
            </div>
              ...
              {payrollPeriods.slice(0, 3).map((period) => (
                <div key={period.id} className="border rounded-lg p-3">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span
                      className={`px-2 py-1 rounded-full text-xs ${getStatusColor(period.status)}`}
                    >
                ...
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    <div>
                      ...
                      {period.total_taxes.toLocaleString()}
                    </div>
                    <div>
                ...
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    <div>
                      ...
                      {period.total_deductions.toLocaleString()}
                    </div>
                    <div>
                  ...
                  <tr key={employee.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-4">
                      <div className="font-medium capitalize">
                        {employee.payType.replace("_", " + ")}
                      </div>
                      <div className="text-sm text-gray-600">
                        {employee.baseSalary
                          ? `$${employee.baseSalary.toLocaleString()}/yr`
                          : employee.hourlyRate
                    ...
                    <td className="p-4">
                      ...
                      {employee.overtimeHours > 0 && (
                        <div className="text-sm text-orange-600">
                          +{employee.overtimeHours.toLocaleString()}h OT
                        </div>
                      )}
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <div className="font-medium">
                        ${employee.ytdGross.toLocaleString()}
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="flex gap-1">
                        ...
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            handleGeneratePayStub(employee.employeeId)
                          }
                        >
                          Pay Stub
                        </Button>
                ...
                {taxSettings.map((tax) => (
                  <tr key={tax.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-4">
                      ...
                      {tax.limit && (
                        <div className="text-sm text-gray-600">
                          Up to ${tax.limit.toLocaleString()}
                        </div>
                      )}
                    </td>
                    <td className="p-4 capitalize">{tax.calculation}</td>
                    <td className="p-4 capitalize">
                      {tax.applicableGross.replace("_", " ")}
                    </td>
                    <td className="p-4">
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${getStatusColor(tax.status)}`}
                      >
            ...
            {deductionTypes.map((deduction) => (
              <div key={deduction.id} className="border rounded-lg p-4">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs ${getStatusColor(deduction.status)}`}
                  >
  ...
  const handleDownloadReport = (reportId) => {
    alert(
      `Downloading report ID: ${reportId}. This would download the selected payroll report.`,
    );
  };
      ...
      <div className="p-6">
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            Manage employee payroll, taxes, deductions, and reports
          </p>
        </div>
            ...
            <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold">
                  Payroll Period - {selectedPayroll.period_name}
                </h2>
                <button
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div>
                        ...
                        <span
                          className={`ml-1 px-2 py-1 rounded-full text-xs ${getStatusColor(selectedPayroll.status)}`}
                        >
                      ...
                      <div>
                        <span className="font-medium">Employees:</span>{" "}
                        {selectedPayroll.employee_count}
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div>
                        ...
                        {selectedPayroll.total_gross.toLocaleString()}
                      </div>
                      <div>
                        <span className="font-medium">Total Deductions:</span> $
                      ...
                      <div>
                        <span className="font-medium">Total Taxes:</span> $
                        {selectedPayroll.total_taxes.toLocaleString()}
                      </div>
                      <div>
                        <span className="font-medium">Total Net:</span> $
                        {selectedPayroll.total_net.toLocaleString()}
                      </div>
                      <div>
                        <span className="font-medium">Bonuses:</span> $
                        {selectedPayroll.total_taxes.toLocaleString()}
                ...
                {selectedPayroll.status === "processing" && (
                  <div>
                    ...
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-sm font-medium">
                        {selectedPayroll.processing_progress}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-3">
                      <div
                        ...
                        style={{
                          width: `${selectedPayroll.processing_progress}%`,
                        }}
                      ></div>
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-lg p-6 max-w-3xl w-full mx-4 max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div>
                        ...
                        {selectedEmployee.payType.replace("_", " + ")}
                      </div>
                      <div>
                        <span className="font-medium">Base Salary:</span>{" "}
                        {selectedEmployee.baseSalary
                          ? `$${selectedEmployee.baseSalary.toLocaleString()}/year`
                          : "N/A"}
                      </div>
                      <div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div>
                        ...
                        {selectedEmployee.hoursWorked.toLocaleString()}h
                      </div>
                      <div>
                        <span className="font-medium">Overtime Hours:</span>{" "}
                        {selectedEmployee.overtimeHours.toLocaleString()}h
                      </div>
                      <div>
                        <span className="font-medium">Gross Pay:</span> $
                      ...
                      <div>
                        <span className="font-medium">Commissions:</span> $
                        {selectedEmployee.commissions.toLocaleString()}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <span className="font-medium">YTD Gross:</span> $
                      {selectedEmployee.ytdGross.toLocaleString()}
                    </div>
                    <div>
                      <span className="font-medium">YTD Net:</span> $
                      {selectedEmployee.ytdNet.toLocaleString()}
                    </div>
                  </div>
                </div>
              </div>
