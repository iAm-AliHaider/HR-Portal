...
async function getLoanApplications(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (error) {
      ...
      if (status && status !== 'all') {
        filteredMockData = filteredMockData.filter(app => app.status === status);
      }
      
      if (employee_id) {
        filteredMockData = filteredMockData.filter(app => app.employee_id === employee_id);
      }
...
async function getLoanAnalytics(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const analytics = {
      ...
      monthly_trends: [
        { month: 'Jan', applications: 28, disbursements: 18, collections: 1100000 },
        { month: 'Feb', applications: 32, disbursements: 22, collections: 1150000 },
        { month: 'Mar', applications: 35, disbursements: 25, collections: 1200000 }
      ]
    };
  ...
  try {
    ...
    if (error) {
      ...
      const mockEmployeeLoans = mockLoanApplications.filter(
        loan => loan.employee_id === employee_id && loan.status === 'approved'
      );
...
async function createLoanApplication(req: NextApiRequest, res: NextApiResponse) {
  const {
    employee_id,
    employee_name,
    employee_email,
  ...
  try {
    ...
    const loanTypeSettings = mockLoanSettings.loan_types.find(lt => lt.id === loan_type);
    if (!loanTypeSettings) {
      return res.status(400).json({ error: 'Invalid loan type' });
    }

    const interest_rate = (loanTypeSettings.interest_rate_min + loanTypeSettings.interest_rate_max) / 2;
    const monthly_rate = interest_rate / 100 / 12;
    const num_payments = term_months;
    const monthly_payment = (amount * monthly_rate * Math.pow(1 + monthly_rate, num_payments)) / 
...
async function approveLoanApplication(req: NextApiRequest, res: NextApiResponse) {
  const { loan_id, approved_amount, interest_rate, notes, approver_id } = req.body;
...
async function rejectLoanApplication(req: NextApiRequest, res: NextApiResponse) {
...
async function disburseLoan(req: NextApiRequest, res: NextApiResponse) {
  const { loan_id, disbursement_date, disbursement_amount, bank_details } = req.body;
...
async function handleDelete(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    return res.status(200).json({ message: 'Loan application deleted successfully' });
  } catch (error) {
