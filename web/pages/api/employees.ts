...
const mockEmployees: Employee[] = [
  ...
  },
  {
    ...
    skills: ['Engineering Leadership', 'Architecture', 'Team Building', 'Strategy'],
...
async function handleGet(req: NextApiRequest, res: NextApiResponse) {
  const { id, search, department, status, page = '1', limit = '10' } = req.query;
  ...
  try {
    ...
    if (error) {
      ...
      if (department) {
        filteredEmployees = filteredEmployees.filter(emp => emp.department_id === department);
      }
...
async function getEmployeeById(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const processedEmployee = {
      ...
      manager_name: data.manager ? `${data.manager.first_name} ${data.manager.last_name}` : null,
