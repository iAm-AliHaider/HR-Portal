...
const mockTeams = [
  {
    ...
    member_count: 4,
    budget: 600000,
    projects: ['API Redesign', 'Database Migration', 'Performance Optimization'],
    created_at: '2024-01-01T00:00:00Z'
  },
  {
    id: 'team-002',
...
async function getDepartments(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const departmentsWithHeadNames = data?.map(dept => ({
      ...dept,
      head_name: dept.head ? `${dept.head.first_name} ${dept.head.last_name}` : null
    }));
...
async function getTeams(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const teamsWithDetails = data?.map(team => ({
      ...
      lead_name: team.lead ? `${team.lead.first_name} ${team.lead.last_name}` : null
    }));
...
async function getOrganizationStructure(req: NextApiRequest, res: NextApiResponse) {
  try {
...
async function updateCompanySettings(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (error) {
      ...
      return res.status(200).json({ ...mockCompanySettings, ...updatedSettings });
    }
