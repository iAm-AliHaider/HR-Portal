...
export default function RecruitmentAnalytics() {
  ...
  const [topDepartments, setTopDepartments] = useState<{department: string, count: number}[]>([]);
  ...
  // Calculate metrics
  useEffect(() => {
    if (jobsLoading || applicationsLoading || interviewsLoading || offersLoading) return;
    
    const cutoffDate = getCutoffDate(timeframe);
    const previousCutoff = getPreviousPeriodCutoff(timeframe);
    
    // Filter data based on timeframe and department
    const filteredJobs = filterByDepartment(jobs).filter(job => new Date(job.created_at) >= cutoffDate);
    const filteredApplications = filterByDepartment(applications).filter(app => new Date(app.applied_at) >= cutoffDate);
    const filteredInterviews = filterByDepartment(interviews).filter(interview => new Date(interview.scheduled_at) >= cutoffDate);
    const filteredOffers = filterByDepartment(offers).filter(offer => new Date(offer.created_at) >= cutoffDate);
    
    // Calculate metrics
    const activeJobs = filteredJobs.filter(job => job.status === 'open').length;
    const newApplications = filteredApplications.filter(app => app.status === 'Applied').length;
    const interviewsScheduled = filteredInterviews.length;
    const interviewsCompleted = filteredInterviews.filter(interview => interview.status === 'completed').length;
    const offersExtended = filteredOffers.length;
    const offersAccepted = filteredOffers.filter(offer => offer.status === 'accepted').length;
    ...
    const screeningToInterview = totalApps > 0 ? (interviewsScheduled / totalApps) * 100 : 0;
    const interviewToOffer = interviewsCompleted > 0 ? (offersExtended / interviewsCompleted) * 100 : 0;
    const offerToHire = offersExtended > 0 ? (offersAccepted / offersExtended) * 100 : 0;
    const overallConversion = totalApps > 0 ? (hires / totalApps) * 100 : 0;
    
    // Calculate previous period rates for comparison
    const prevApplications = filterByDepartment(applications)
      .filter(app => new Date(app.applied_at) >= previousCutoff && new Date(app.applied_at) < cutoffDate);
    const prevInterviews = filterByDepartment(interviews)
      .filter(interview => new Date(interview.scheduled_at) >= previousCutoff && new Date(interview.scheduled_at) < cutoffDate);
    const prevOffers = filterByDepartment(offers)
      .filter(offer => new Date(offer.created_at) >= previousCutoff && new Date(offer.created_at) < cutoffDate);
    
    const prevScreeningToInterview = prevApplications.length > 0 
      ? (prevInterviews.length / prevApplications.length) * 100 
      : 0;
    const prevInterviewToOffer = prevInterviews.filter(i => i.status === 'completed').length > 0 
      ? (prevOffers.length / prevInterviews.filter(i => i.status === 'completed').length) * 100 
      : 0;
    const prevOfferToHire = prevOffers.length > 0 
      ? (prevOffers.filter(o => o.status === 'accepted').length / prevOffers.length) * 100 
      : 0;
    const prevOverallConversion = prevApplications.length > 0 
      ? (prevOffers.filter(o => o.status === 'accepted').length / prevApplications.length) * 100 
      : 0;
    ...
    const sources = Array.from(new Set(filteredApplications.map(app => app.source || 'Direct')));
    const sourceData = sources.map(source => {
      const sourceApps = filteredApplications.filter(app => (app.source || 'Direct') === source);
      const sourceAppIds = sourceApps.map(app => app.id);
      const sourceInterviews = filteredInterviews.filter(i => sourceAppIds.includes(i.application_id));
      const sourceOffers = filteredOffers.filter(o => sourceAppIds.includes(o.application_id));
      ...
      return {
        ...
        conversionRate: sourceApps.length > 0 ? (sourceHires.length / sourceApps.length) * 100 : 0
      };
    ...
    const departments = Array.from(new Set(applications.map(app => app.department || 'General')));
    const timeToHireData = departments.map(dept => {
      const deptApps = applications.filter(app => (app.department || 'General') === dept && app.status === 'Hired');
      
      // Calculate average days to hire for this department
      const daysToHire = deptApps.length > 0 
        ? deptApps.reduce((sum, app) => {
            const appliedDate = new Date(app.applied_at);
            const hiredDate = new Date(app.hired_at || app.updated_at);
            return sum + Math.round((hiredDate.getTime() - appliedDate.getTime()) / (1000 * 60 * 60 * 24));
          }, 0) / deptApps.length
        : 0;
      
      // Calculate previous period
      const prevDeptApps = applications.filter(
        app => (app.department || 'General') === dept && 
        app.status === 'Hired' &&
        new Date(app.applied_at) >= previousCutoff && 
        new Date(app.applied_at) < cutoffDate
      );
      
      const prevDaysToHire = prevDeptApps.length > 0 
    ...
    }).filter(d => d.daysToHire > 0).sort((a, b) => a.daysToHire - b.daysToHire);
    
    setTimeToHire(timeToHireData);
    
    // Calculate top departments by hiring
    const deptHires = departments.map(dept => {
  ...
  }, [jobs, applications, interviews, offers, timeframe, department, jobsLoading, applicationsLoading, interviewsLoading, offersLoading]);
        ...
        <div className="mb-6 flex flex-col md:flex-row justify-between gap-4">
          ...
          <div className="bg-blue-50 text-blue-800 px-4 py-2 rounded-md border border-blue-200 flex items-center">
            <span className="text-sm font-medium">
              Showing data for: {getTimeframeText()} {department !== 'all' ? ` • ${department}` : ''}
            </span>
          </div>
        </div>
        
        {jobsLoading || applicationsLoading || interviewsLoading || offersLoading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card>
                <CardContent className="pt-6">
                  <div className="flex flex-col items-center text-center">
                    <h3 className="text-lg font-medium text-gray-500">Open Positions</h3>
                    <p className="text-3xl font-bold mt-2">{metrics.activeJobs}</p>
                    <p className="text-sm text-gray-500 mt-1">of {metrics.totalJobs} total</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="pt-6">
                  <div className="flex flex-col items-center text-center">
                    ...
                    <p className="text-3xl font-bold mt-2">{metrics.totalApplications}</p>
                    <p className="text-sm text-gray-500 mt-1">{metrics.newApplications} new</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="pt-6">
                  <div className="flex flex-col items-center text-center">
                    <h3 className="text-lg font-medium text-gray-500">Interviews</h3>
                    <p className="text-3xl font-bold mt-2">{metrics.interviewsScheduled}</p>
                    <p className="text-sm text-gray-500 mt-1">{metrics.interviewsCompleted} completed</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="pt-6">
                  <div className="flex flex-col items-center text-center">
                    ...
                    <p className="text-sm text-gray-500 mt-1">of {metrics.offersExtended} offers</p>
                  </div>
                </CardContent>
              </Card>
            </div>
                    ...
                    <div key={index} className="space-y-2">
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center">
                          <span className="text-xl font-bold">{rate.rate.toFixed(1)}%</span>
                          {rate.change !== 0 && (
                            <span className={`ml-2 text-xs ${rate.change > 0 ? 'text-green-600' : 'text-red-600'}`}>
                    ...
                    <tbody>
                      {sourceEffectiveness.map((source, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                          ...
                          <td className="py-3 text-right">{source.interviews}</td>
                          <td className="py-3 text-right">{source.offers}</td>
                          <td className="py-3 text-right">{source.hires}</td>
                          <td className="py-3 text-right">
                            <span className={`font-medium ${
                              source.conversionRate > 5 ? 'text-green-600' : 
                              source.conversionRate > 2 ? 'text-blue-600' : 
                              'text-gray-600'
                            }`}>
                            ...
                            <div className="flex items-center">
                              ...
                              {item.change !== 0 && (
                                <span className={`ml-2 text-xs ${item.change < 0 ? 'text-green-600' : 'text-red-600'}`}>
                                  {item.change < 0 ? '↓' : '↑'} {Math.abs(Math.round(item.change))} days
                                </span>
                              )}
                            </div>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2.5">
                            <div 
                              className={`h-2.5 rounded-full ${
                                item.daysToHire < 30 ? 'bg-green-500' : 
                                item.daysToHire < 60 ? 'bg-yellow-500' : 
                                'bg-red-500'
                              }`}
                              style={{ width: `${Math.min(100, (item.daysToHire / 90) * 100)}%` }}
                            ></div>
                          </div>
                        </div>
                      ))}
                      ...
                      {topDepartments.map((dept, index) => (
                        <div key={index} className="space-y-2">
                          <div className="flex justify-between">
                            ...
                            <span>{dept.count} {dept.count === 1 ? 'hire' : 'hires'}</span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2.5">
                            <div 
                              className="bg-blue-600 h-2.5 rounded-full" 
