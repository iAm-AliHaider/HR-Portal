...
export default function RecruitmentDashboard() {
  ...
  const { applications: allApplications, loading: applicationsLoading } =
    useApplications();
  ...
  const inScreening = allApplications.filter(
    (app) => app.status === "Screening",
  ).length;
  const inInterview = allApplications.filter(
    (app) => app.status === "Interview",
  ).length;
  const inOffer = allApplications.filter(
  ...
  const screeningRate =
    totalApplications > 0
      ? Math.round((inScreening / totalApplications) * 100)
      : 0;
  const interviewRate =
    inScreening > 0 ? Math.round((inInterview / inScreening) * 100) : 0;
  const offerRate =
    inInterview > 0 ? Math.round((inOffer / inInterview) * 100) : 0;
      ...
      >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <div className="rounded-md border border-zinc-200 bg-white p-6 shadow-sm">
              <h3 className="text-lg font-medium text-zinc-900 mb-4 flex items-center">
                <BarChart2
                  className="mr-2 h-5 w-5 text-zinc-700"
                  strokeWidth={1.5}
                />
                Recruitment Funnel
              </h3>
              <div className="space-y-4">
                ...
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-sm font-medium text-zinc-900">
                      {inScreening} ({screeningRate}%)
                    </span>
                  </div>
                  <div className="w-full bg-zinc-200 rounded-full h-2.5">
                    <div
                      className="bg-zinc-800 h-2.5 rounded-full"
                ...
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-sm font-medium text-zinc-900">
                      {inInterview} ({interviewRate}%)
                    </span>
                  </div>
                  <div className="w-full bg-zinc-200 rounded-full h-2.5">
                    <div
                      className="bg-zinc-700 h-2.5 rounded-full"
              ...
              <div className="space-y-4">
                {allApplications.slice(0, 5).map((application, index) => (
                  <div key={index} className="flex items-start">
                    <div
                      className={`w-3 h-3 mt-1.5 rounded-full mr-3 ${
          ...
          <div className="grid grid-cols-1 gap-6">
            <div className="rounded-md border border-zinc-200 bg-white p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                >
                  <Plus
                    className="w-6 h-6 text-zinc-900 mb-2"
                    strokeWidth={1.5}
                  />
                ...
                {jobs.map((job) => (
                  ...
                  >
                    ...
                    <div className="mt-2 flex items-center text-sm">
                      ...
                      <ArrowRight
                        className="h-3 w-3 ml-auto text-zinc-400"
                        strokeWidth={1.5}
                      />
                    </div>
                  </div>
        ...
        <TabsContent value="interviews">
          <div className="mb-6 text-center">
            <Calendar
              className="h-16 w-16 text-zinc-300 mx-auto mb-2"
              strokeWidth={1.5}
            />
        ...
        <TabsContent value="offers">
          <div className="mb-6 text-center">
            ...
            <Button onClick={() => router.push("/offers")}>View Offers</Button>
          </div>
        </TabsContent>
      </Tabs>
    </PageLayout>
