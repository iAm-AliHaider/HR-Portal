  ...
  const loadReportData = async () => {
    try {
      ...
      const utilizationRate =
        totalBookings > 0
          ? Math.round((totalBookings / (rooms.length + assets.length)) * 100)
          : 0;

      // Top used rooms
      const roomBookingCounts = roomBookings.reduce(
        (acc, booking) => {
      ...
      const topUsedRooms = rooms
        .map((room) => ({
          ...
          utilization: Math.round(
            ((roomBookingCounts[room.id] || 0) / roomBookings.length) * 100,
          ),
        }))
      ...
      // Top used assets
      const assetBookingCounts = assetBookings.reduce(
        (acc, booking) => {
      ...
      const topUsedAssets = assets
        .map((asset) => ({
          ...
          utilization: Math.round(
            ((assetBookingCounts[asset.id] || 0) / assetBookings.length) * 100,
          ),
        }))
        .sort((a, b) => b.bookings - a.bookings)
        .slice(0, 5);
      ...
      const categoryCount = assets.reduce(
        (acc, asset) => {
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Analytics and reports for facilities and resource management"
        />
      </Head>
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-gray-600">
                Insights and analytics for facilities and resource management
              </p>
            </div>
                            ...
                            <div className="flex-1">
                              <div className="bg-gray-200 rounded-full h-2">
                                <div
                                  ...
                                  style={{
                                    width: `${Math.min((month.rooms / Math.max(...metrics.bookingsByMonth.map((m) => m.rooms))) * 100, 100)}%`,
                                  }}
                                ></div>
                              </div>
                            ...
                            <div className="flex-1">
                              <div className="bg-gray-200 rounded-full h-2">
                                <div
                                  ...
                                  style={{
                                    width: `${Math.min((month.assets / Math.max(...metrics.bookingsByMonth.map((m) => m.assets))) * 100, 100)}%`,
                                  }}
                                ></div>
                              </div>
                              <span className="text-xs text-gray-500">
                                Equipment: {month.assets}
                              </span>
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="flex items-center space-x-3">
                            <div className="w-24 bg-gray-200 rounded-full h-2">
                              <div
                                className="bg-blue-500 h-2 rounded-full"
                                style={{
                                  width: `${(category.count / Math.max(...metrics.categoryDistribution.map((c) => c.count))) * 100}%`,
                                }}
                              ></div>
                            </div>
                    ...
                    {metrics.statusDistribution.map((status, index) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center space-x-3">
                            <div
                              className={`w-3 h-3 rounded-full ${colors[status.status as keyof typeof colors] || "bg-gray-500"}`}
                            ></div>
                            <span className="text-sm font-medium text-gray-700 capitalize">
