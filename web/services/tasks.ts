...
import { Task, TaskComment, TaskAttachment, TaskChecklistItem } from '../../packages/types';
import { v4 as uuidv4 } from 'uuid';

// Mock data for development
const mockUsers = [
  { id: 'user1', full_name: 'John Doe', email: 'john@example.com', avatar: '/avatars/john.jpg' },
  { id: 'user2', full_name: 'Jane Smith', email: 'jane@example.com', avatar: '/avatars/jane.jpg' },
  { id: 'user3', full_name: 'Robert Johnson', email: 'robert@example.com', avatar: '/avatars/robert.jpg' },
  { id: 'user4', full_name: 'Emily Davis', email: 'emily@example.com', avatar: '/avatars/emily.jpg' },
];
...
const mockTasks: Task[] = [
  {
    ...
    description: 'Review team performance metrics and prepare summary for executive meeting',
  ...
  },
  {
    ...
    description: 'Incorporate new remote work policy into the employee handbook',
  ...
  },
  {
    ...
    description: 'Create welcome package for new developers starting next month',
  ...
  },
  {
    ...
    description: 'Plan and schedule a team building activity for the engineering department',
  ...
  },
  {
    ...
    description: 'Review and process expense reports for the marketing department',
  ...
  },
  {
    ...
    recurrence_end_date: new Date(Date.now() + 86400000 * 365 * 3).toISOString(), // 3 years from now
    estimated_hours: 5,
  },
  {
    id: 'task7',
    ...
    description: 'Create and distribute satisfaction survey to all active clients',
  ...
  },
  {
    ...
    description: 'Prepare and distribute weekly team status updates to stakeholders',
  ...
  },
  {
    ...
    description: 'Review and update the company website content for accuracy and relevance',
  ...
  },
  {
    ...
    description: 'Comprehensive refresh of the company website including content, design, and functionality',
...
export const addTaskComment = async (taskId: string, comment: Omit<TaskComment, 'id' | 'created_at'>): Promise<TaskComment> => {
  try {
...
export const addTaskChecklistItem = async (taskId: string, item: Omit<TaskChecklistItem, 'id'>): Promise<TaskChecklistItem> => {
  try {
    ...
    console.error(`Error adding checklist item to task with ID ${taskId}:`, error);
    throw error;
  }
};
...
export const updateTaskChecklistItem = async (
  ...
  try {
    ...
    return new Promise(resolve => {
      setTimeout(() => {
        const task = mockTasks.find(t => t.id === taskId);
        if (!task || !task.checklist) throw new Error(`Task with ID ${taskId} or checklist not found`);
        
        const index = task.checklist.findIndex(i => i.id === itemId);
        if (index === -1) throw new Error(`Checklist item with ID ${itemId} not found`);
...
export const getTaskCategories = (): string[] => {
  return ['HR', 'Documentation', 'Onboarding', 'Events', 'Finance', 'Compliance', 'Client Relations', 'Development', 'Marketing'];
};
