...
import { Interview, InterviewFeedback, Application } from '../../packages/types';
...
export async function getInterviewsByApplicationId(applicationId: string): Promise<Interview[]> {
  try {
    ...
    console.warn('Supabase query failed, returning mock data:', error);
    return mockInterviews.filter(interview => interview.application_id === applicationId);
  }
}
...
export async function scheduleInterview(interview: Partial<Interview> & {
  booking_options?: {
    room_id?: string;
    asset_ids?: string[];
    booked_by: string;
    ...
    if (interview.type === 'in_person' && interview.booking_options) {
      try {
        ...
        if (bookings.assetBookings.length > 0) {
          bookingUpdates.asset_booking_ids = bookings.assetBookings.map(b => b.id);
        }
...
export async function rescheduleInterview(
  ...
  try {
    ...
    await BookingService.cancelBookingsForInterview(id, bookingOptions?.booked_by || 'system');
    ...
    if (currentInterview.type === 'in_person' && bookingOptions) {
      try {
        ...
        const endTime = new Date(startTime.getTime() + (currentInterview.duration || 60) * 60000);
        ...
        console.warn('Failed to create new bookings for rescheduled interview:', bookingError);
      }
    }
...
export async function cancelInterview(id: string, reason?: string, cancelledBy?: string): Promise<Interview> {
...
export async function getInterviewFeedback(interviewId: string): Promise<InterviewFeedback[]> {
  try {
    const { data, error } = await supabase
...
export async function getInterviewsForInterviewer(interviewerId: string): Promise<Interview[]> {
  try {
    const { data, error } = await supabase
      .from('interviews')
      .select('*')
...
export async function getUpcomingInterviews(orgId: string, days: number = 7): Promise<Interview[]> {
  try {
...
export function getInterviewTypeOptions() {
  return [
    { value: 'phone', label: 'Phone Interview', description: 'Voice-only interview conducted over phone' },
    { value: 'video', label: 'Video Interview', description: 'Video call interview using Zoom, Teams, etc.' },
    { value: 'in_person', label: 'In-Person Interview', description: 'Face-to-face interview at office location' },
    { value: 'technical', label: 'Technical Interview', description: 'Technical assessment with coding/practical tasks' },
    { value: 'panel', label: 'Panel Interview', description: 'Interview conducted by multiple interviewers' }
  ];
}
