  ...
  async supabaseCall<T>(
    ...
    try {
      ...
      try {
        ...
        if ((this.isDevelopment && mockData) || !supabase) {
          console.warn('Supabase operation failed, using mock data instead:', operationError);
          return {
            data: mockData,
            error: null,
            loading: false
...
class EmployeeService extends ApiService {
  ...
  async createEmployee(employeeData: any): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('employees').insert(employeeData).select().single(),
      mockCreatedEmployee
    );
  }
  ...
  async updateEmployee(id: string, updates: any): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('employees').update(updates).eq('id', id).select().single(),
      mockUpdatedEmployee
    );
  }
  ...
  async getEmployeeById(id: string): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('employees').select('*').eq('id', id).single(),
      mockEmployee
    );
  }
}
...
class JobService extends ApiService {
  ...
  async updateJob(id: string, updates: any): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('jobs').update(updates).eq('id', id).select().single(),
      mockUpdatedJob
    );
  }

  async closeJob(id: string): Promise<ApiResponse<any>> {
    return this.updateJob(id, { status: 'closed', closed_at: new Date().toISOString() });
  }
  ...
  async getApplicationsForJob(jobId: string): Promise<ApiResponse<any[]>> {
    const mockJobApplications = mockApplications.filter(app => app.job_id === jobId);
    return this.supabaseCall(
      async () => await supabase.from('applications').select('*').eq('job_id', jobId),
      mockJobApplications
    );
  }
}
...
class LeaveService extends ApiService {
  ...
  async submitLeaveRequest(leaveData: any): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('leave_requests').insert(leaveData).select().single(),
      mockSubmittedRequest
    );
  }

  async approveLeaveRequest(id: string, notes?: string): Promise<ApiResponse<any>> {
    const updates = {
      status: 'approved',
    ...
    return this.supabaseCall(
      async () => await supabase.from('leave_requests').update(updates).eq('id', id).select().single(),
      { id, ...updates }
    );
  }
...
class TrainingService extends ApiService {
  ...
  async enrollInCourse(courseId: string, employeeId: string): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('course_enrollments').insert({
        course_id: courseId,
        employee_id: employeeId
      }).select().single(),
      mockEnrollment
  ...
  async createCourse(courseData: any): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('training_courses').insert(courseData).select().single(),
      mockCreatedCourse
    );
  }
}
...
class ComplianceService extends ApiService {
  ...
  async startAudit(requirementId: string): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('audits').insert({
        requirement_id: requirementId,
        status: 'in_progress'
      }).select().single(),
      mockAudit
  ...
  async updateComplianceStatus(id: string, status: string): Promise<ApiResponse<any>> {
    const updates = {
      status,
    ...
    return this.supabaseCall(
      async () => await supabase.from('compliance_requirements').update(updates).eq('id', id).select().single(),
      { id, ...updates }
    );
  }
}
...
class WorkflowService extends ApiService {
  ...
  async createWorkflow(workflowData: any): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('workflows').insert(workflowData).select().single(),
      mockCreatedWorkflow
    );
  }

  async startWorkflow(workflowId: string, contextData: any): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('workflow_instances').insert({
        workflow_id: workflowId,
        context_data: contextData
      }).select().single(),
...
class PerformanceService extends ApiService {
  ...
  async createReview(reviewData: any): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('performance_reviews').insert(reviewData).select().single(),
      mockCreatedReview
    );
  }
  ...
  async submitReview(id: string, reviewData: any): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('performance_reviews').update(updates).eq('id', id).select().single(),
      { id, ...updates }
    );
  }
}
...
class ApplicationService extends ApiService {
  ...
  async createApplication(applicationData: any): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('applications').insert(applicationData).select().single(),
      mockCreatedApplication
    );
  }

  async updateApplicationStatus(id: string, status: string, notes?: string): Promise<ApiResponse<any>> {
    const updates = {
      status,
      notes,
      updated_at: new Date().toISOString()
    ...
    return this.supabaseCall(
      async () => await supabase.from('applications').update(updates).eq('id', id).select().single(),
      { id, ...updates }
    );
  }

  async scheduleInterview(applicationId: string, interviewData: any): Promise<ApiResponse<any>> {
    const mockInterview = {
      id: Date.now().toString(),
    ...
    return this.supabaseCall(
      async () => await supabase.from('interviews').insert({
        application_id: applicationId,
        ...interviewData
      }).select().single(),
      mockInterview
...
class ExpenseService extends ApiService {
  ...
  async submitExpense(expenseData: any): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('expenses').insert(expenseData).select().single(),
  ...
  async approveExpense(id: string): Promise<ApiResponse<any>> {
    ...
    return this.supabaseCall(
      async () => await supabase.from('expenses').update(updates).eq('id', id).select().single(),
      { id, ...updates }
    );
  }
...
export const scheduleInterview = async (interviewData: any): Promise<ApiResponse<any>> => {
  try {
    if (process.env.NODE_ENV === 'development') {
      // Simulate API delay
      await new Promise(resolve => setTimeout(resolve, 800));
    ...
    return { data: null, error: error instanceof Error ? error.message : 'Failed to schedule interview', loading: false };
  }
};

export const updateInterviewStatus = async (id: string, status: string, notes?: string): Promise<ApiResponse<any>> => {
  try {
    if (process.env.NODE_ENV === 'development') {
      // Simulate API delay
      await new Promise(resolve => setTimeout(resolve, 800));
      ...
      return { data: mockInterviews[interviewIndex], error: null, loading: false };
    } else {
      // Production code would use Supabase or another API
      const { data, error } = await supabase
        .from('interviews')
    ...
    return { data: null, error: error instanceof Error ? error.message : 'Failed to update interview status', loading: false };
  }
};
...
export const createOffer = async (offerData: any): Promise<ApiResponse<any>> => {
  try {
    if (process.env.NODE_ENV === 'development') {
      // Simulate API delay
      await new Promise(resolve => setTimeout(resolve, 800));
    ...
    return { data: null, error: error instanceof Error ? error.message : 'Failed to create offer', loading: false };
  }
};

export const updateOfferStatus = async (id: string, status: string, notes?: string): Promise<ApiResponse<any>> => {
  try {
    if (process.env.NODE_ENV === 'development') {
      // Simulate API delay
      await new Promise(resolve => setTimeout(resolve, 800));
    ...
    return { data: null, error: error instanceof Error ? error.message : 'Failed to update offer status', loading: false };
  }
};
...
export const notificationService = {
  async send(type: 'email' | 'sms' | 'push', recipient: string, message: string) {
    if (process.env.NODE_ENV === 'development') {
...
export const fileService = {
  async upload(file: File, folder: string = 'documents'): Promise<ApiResponse<{ url: string; name: string }>> {
    if (process.env.NODE_ENV === 'development') {
      // Mock file upload
