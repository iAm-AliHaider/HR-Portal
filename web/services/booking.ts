...
import { MeetingRoom, Asset, RoomBooking, AssetBooking, BookingConflict } from '../../packages/types/hr'
...
const mockMeetingRooms: MeetingRoom[] = [
  ...
  },
  {
    ...
    video_conference_enabled: true,
    accessibility_features: ['wheelchair_accessible', 'hearing_loop', 'braille_signage'],
    created_at: '2024-01-01T00:00:00Z',
    updated_at: '2024-01-01T00:00:00Z'
  }
];
...
const mockRoomBookings: RoomBooking[] = [
  ...
  },
  {
    ...
    start_time: new Date(new Date(new Date().setDate(new Date().getDate() + 1)).setHours(16, 0, 0, 0)).toISOString(),
    end_time: new Date(new Date(new Date().setDate(new Date().getDate() + 1)).setHours(18, 0, 0, 0)).toISOString(),
    attendee_count: 12,
...
const mockAssetBookings: AssetBooking[] = [
  {
    ...
    end_time: new Date(new Date(new Date().setDate(new Date().getDate() + 7)).setHours(17, 0, 0, 0)).toISOString(),
  ...
  },
  {
    ...
    start_time: new Date(new Date(new Date().setDate(new Date().getDate() + 2)).setHours(13, 0, 0, 0)).toISOString(),
    end_time: new Date(new Date(new Date().setDate(new Date().getDate() + 2)).setHours(17, 0, 0, 0)).toISOString(),
  ...
  },
  {
    ...
    start_time: new Date(new Date(new Date().setDate(new Date().getDate() + 5)).setHours(18, 0, 0, 0)).toISOString(),
    end_time: new Date(new Date(new Date().setDate(new Date().getDate() + 6)).setHours(22, 0, 0, 0)).toISOString(),
...
const isSupabaseConfigured = () => {
  try {
    ...
    return url && key && url !== 'https://example.supabase.co' && !key.includes('example');
  } catch (e) {
    return false;
  }
};
...
export const BookingService = {
  ...
  async getMeetingRooms(orgId: string, filters?: {
    is_active?: boolean;
    capacity_min?: number;
    location?: string;
    equipment?: string[];
    ...
    if (!isSupabaseConfigured()) {
      await mockDelay();
      let filteredRooms = mockMeetingRooms.filter(room => room.org_id === orgId);
      
      if (filters?.is_active !== undefined) {
        filteredRooms = filteredRooms.filter(room => room.is_active === filters.is_active);
      }
      if (filters?.capacity_min) {
        filteredRooms = filteredRooms.filter(room => room.capacity >= filters.capacity_min);
      }
      if (filters?.location) {
  ...
  async updateMeetingRoom(roomId: string, updates: Partial<MeetingRoom>): Promise<MeetingRoom> {
    if (!isSupabaseConfigured()) {
      await mockDelay();
      const roomIndex = mockMeetingRooms.findIndex(room => room.id === roomId);
  ...
  async deleteMeetingRoom(roomId: string): Promise<void> {
    if (!isSupabaseConfigured()) {
      ...
      const activeBookings = mockRoomBookings.filter(booking => 
  ...
  async getAssets(orgId: string, filters?: {
    ...
    if (!isSupabaseConfigured()) {
      ...
      if (filters?.available_only) {
        filteredAssets = filteredAssets.filter(asset => asset.status === 'available');
      }
  ...
  async deleteAsset(assetId: string): Promise<void> {
    if (!isSupabaseConfigured()) {
      ...
      const activeBookings = mockAssetBookings.filter(booking => 
  ...
  async checkRoomAvailability(roomId: string, startTime: string, endTime: string, excludeBookingId?: string): Promise<boolean> {
    if (!isSupabaseConfigured()) {
      await mockDelay(100);
  ...
  async updateRoomBooking(bookingId: string, updates: Partial<RoomBooking>): Promise<RoomBooking> {
    if (!isSupabaseConfigured()) {
      await mockDelay();
      const bookingIndex = mockRoomBookings.findIndex(booking => booking.id === bookingId);
  ...
  async getRoomBookings(orgId: string, filters?: {
    room_id?: string;
    booked_by?: string;
    status?: string;
    start_date?: string;
    ...
    if (!isSupabaseConfigured()) {
      await mockDelay();
      let filteredBookings = mockRoomBookings.filter(booking => booking.org_id === orgId);
      
      if (filters?.room_id) {
        filteredBookings = filteredBookings.filter(booking => booking.room_id === filters.room_id);
      }
      if (filters?.booked_by) {
        filteredBookings = filteredBookings.filter(booking => booking.booked_by === filters.booked_by);
      }
      if (filters?.status) {
        filteredBookings = filteredBookings.filter(booking => booking.status === filters.status);
      }
      if (filters?.booking_type) {
        filteredBookings = filteredBookings.filter(booking => booking.booking_type === filters.booking_type);
      }
      ...
      return filteredBookings.sort((a, b) => new Date(a.start_time).getTime() - new Date(b.start_time).getTime());
    }
  ...
  async checkAssetAvailability(assetId: string, startTime: string, endTime: string, excludeBookingId?: string): Promise<boolean> {
    if (!isSupabaseConfigured()) {
      await mockDelay(100);
  ...
  async createAssetBooking(booking: Partial<AssetBooking>): Promise<AssetBooking> {
    if (!isSupabaseConfigured()) {
      await mockDelay();
      ...
      if (booking.asset_id && booking.start_time && booking.end_time) {
        ...
        if (!isAvailable) {
          throw new Error('Asset is not available during the requested time slot');
        }
      }
  ...
  async updateAssetBooking(bookingId: string, updates: Partial<AssetBooking>): Promise<AssetBooking> {
    if (!isSupabaseConfigured()) {
      await mockDelay();
      const bookingIndex = mockAssetBookings.findIndex(booking => booking.id === bookingId);
      if (bookingIndex === -1) throw new Error('Booking not found');
  ...
  async getAssetBookings(orgId: string, filters?: {
    asset_id?: string;
    booked_by?: string;
    status?: string;
    start_date?: string;
    ...
    if (!isSupabaseConfigured()) {
      await mockDelay();
      let filteredBookings = mockAssetBookings.filter(booking => booking.org_id === orgId);
      
      if (filters?.asset_id) {
        filteredBookings = filteredBookings.filter(booking => booking.asset_id === filters.asset_id);
      }
      if (filters?.booked_by) {
        filteredBookings = filteredBookings.filter(booking => booking.booked_by === filters.booked_by);
      }
  ...
  async checkoutAsset(bookingId: string, checkoutData: {
  ...
  async returnAsset(bookingId: string, returnData: {
    returned_by: string;
    return_notes?: string;
  ...
  // Booking conflicts
  async getBookingConflicts(orgId: string, resolved?: boolean): Promise<BookingConflict[]> {
    if (!isSupabaseConfigured()) {
      await mockDelay();
      let filteredConflicts = mockBookingConflicts.filter(conflict => conflict.org_id === orgId);
      
      if (resolved !== undefined) {
        filteredConflicts = filteredConflicts.filter(conflict => conflict.resolved === resolved);
      }
      
      return filteredConflicts.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    }
  ...
  async resolveBookingConflict(conflictId: string, resolution: {
    ...
    if (!isSupabaseConfigured()) {
      await mockDelay();
      const conflictIndex = mockBookingConflicts.findIndex(conflict => conflict.id === conflictId);
  ...
  async getAssetSchedule(assetId: string, date: string): Promise<AssetBooking[]> {
    const startOfDay = new Date(date)
    startOfDay.setHours(0, 0, 0, 0)
  ...
  async bookResourcesForInterview(interviewId: string, resources: {
    room_id?: string;
    asset_ids?: string[];
    title: string;
    start_time: string;
  ...
  async cancelBookingsForInterview(interviewId: string, cancelledBy: string): Promise<void> {
    if (!isSupabaseConfigured()) {
      await mockDelay();
      
      // Cancel room bookings
      mockRoomBookings.forEach(booking => {
        if (booking.related_record_id === interviewId && booking.booking_type === 'interview') {
