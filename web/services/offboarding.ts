...
export const OffboardingService = {
  ...
  async generateTasksFromTemplate(processId: string, templateId: string) {
    ...
    const tasksToCreate = template.task_templates.map(
        ...
        return {
          ...
          dependencies:
            template.dependencies?.map(
              (depIndex: number) => index + depIndex,
            ) || [],
  ...
  async checkAndAssignDependentTasks(
    processId: string,
  ...
  async completeKnowledgeTransferSession(
    sessionId: string,
    data: {
    ...
    const { data: session, error } = await supabase
      ...
      .update({
        ...
        documentation_created: data.documentation_links
          ? data.documentation_links.length > 0
          : false,
  ...
  async markEquipmentItemReturned(processId: string, itemName: string) {
    ...
    const updatedEquipment =
      process.equipment_to_return?.map((item: any) =>
        item.item === itemName
...
export async function getOffboardingProcesses(
  org_id: string,
  filters?: {
    status?: string;
    department?: string;
...
export async function getOffboardingTemplates(
  ...
  if (filters?.reason_for_leaving)
    query = query.eq("reason_for_leaving", filters.reason_for_leaving);
...
export async function getKnowledgeTransferSessions(
  org_id: string,
  filters?: {
    status?: string;
  ...
  if (filters?.status) query = query.eq("status", filters.status);
  if (filters?.departing_employee_id)
    query = query.eq("departing_employee_id", filters.departing_employee_id);
