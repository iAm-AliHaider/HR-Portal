...
class StorageService {
  ...
  private generateFilePath(file: File, category: string, userId?: string): string {
  ...
  async uploadFile(
    ...
    try {
      ...
      const filePath = customPath || this.generateFilePath(file, category, userId);
  ...
  async uploadMultipleFiles(
    ...
    category: keyof typeof this.configs,
    userId?: string
  ): Promise<{ uploaded: FileMetadata[]; failed: Array<{ file: string; error: string }> }> {
    ...
    results.forEach((result, index) => {
      ...
      if (result.status === 'fulfilled' && result.value.success && result.value.file) {
    ...
    console.log(`📁 Batch upload results: ${uploaded.length} uploaded, ${failed.length} failed`);
    return { uploaded, failed };
  }

  // Delete file
  async deleteFile(filePath: string, bucket?: string): Promise<{ success: boolean; error?: string }> {
    try {
      if (this.isDevelopment) {
  ...
  async uploadTrainingMaterial(file: File, courseId: string): Promise<UploadResult> {
    return this.uploadFile(file, 'training', courseId);
  }

  async uploadExpenseReceipt(file: File, expenseId: string): Promise<UploadResult> {
    return this.uploadFile(file, 'expenses', expenseId);
  }

  async uploadComplianceDocument(file: File, userId?: string): Promise<UploadResult> {
