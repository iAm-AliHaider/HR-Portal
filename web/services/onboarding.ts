...
export const OnboardingService = {
  ...
  async generateTasksFromPlan(workflowId: string, planId: string) {
    ...
    const tasksToCreate = plan.task_templates.map(
      (template: any, index: number) => ({
        ...
        dependencies:
          template.dependencies?.map((depIndex: number) => index + depIndex) ||
          [],
  ...
  async completeMilestone(workflowId: string, milestoneName: string) {
    ...
    const updatedMilestones =
      workflow.milestones?.map((milestone: any) =>
...
export async function getOnboardingWorkflows(
  org_id: string,
  filters?: {
    status?: string;
    department?: string;
