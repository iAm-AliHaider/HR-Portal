              ...
              {activityIdx !== displayActivities.length - 1 && (
                <span
                  className={cn(
                    ...
                    colorBorder && activity.iconColor ? activity.iconColor.replace('text-', 'bg-') + '/20' : ''
                  )}
                  aria-hidden="true"
                />
              )}
              <div className={cn('relative flex items-start', compact ? 'space-x-2' : 'space-x-3')}>
                {/* Activity Icon */}
                {activity.icon ? (
                  <div className={cn(
                  ...
                  <div className={cn(
                    ...
                    compact ? 'h-8 w-8' : 'h-10 w-10',
                    activity.iconColor ? activity.iconColor.replace('text-', 'bg-') + '/20' : 'bg-gray-100'
                  )}>
                    <div className={cn(
                ...
                {/* Activity Content */}
                <div className={cn('min-w-0 flex-1', compact ? 'pt-0' : 'pt-1.5')}>
                  <div className={cn(
                    'text-sm',
                    ...
                    <span className="font-medium text-gray-900">
                      {activity.user ? (
                        activity.user.href ? (
                          <Link href={activity.user.href} className="hover:underline">
                            {activity.user.name}
                          </Link>
                        ) : (
                          activity.user.name
                      ...
                      <>
                        <span className="font-medium text-gray-900">
                          {activity.target.href ? (
                            <Link href={activity.target.href} className="hover:underline">
