...
const ApplicationsList = ({
  ...
  const [displayMode, setDisplayMode] = useState<ViewMode>(
    viewMode === "candidates" ? "cards" : "cards",
  );
  ...
  const {
    ...
    onOpen: onScheduleInterviewOpen,
    onClose: onScheduleInterviewClose,
  } = useDisclosure();
  const {
    isOpen: isBulkEmailOpen,
    onOpen: onBulkEmailOpen,
  ...
  const candidateData: Record<string, User> = {
    user1: {
      ...
      avatar_url:
        "https://images.unsplash.com/photo-1494790108755-2616b612d5ca?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80",
    },
    user2: {
      id: "user2",
      org_id: "org1",
  ...
  const filteredApplications = applications
    ...
    .sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case "last_activity":
          aValue = new Date(
            a.last_activity_date || a.application_date,
          ).getTime();
          bValue = new Date(
            b.last_activity_date || b.application_date,
          ).getTime();
          break;
        case "candidate_name":
  ...
  const stats = {
    ...
    inProgress: applications.filter((a) =>
      ["screening", "interview", "assessment"].includes(a.status),
    ).length,
    shortlisted: applications.filter((a) => a.status === "shortlisted").length,
    hired: applications.filter((a) => a.status === "hired").length,
    rejected: applications.filter((a) => a.status === "rejected").length,
    avgScore:
      applications.reduce((sum, app) => sum + (app.total_score || 0), 0) /
        ...
        <Flex>
          ...
          <Box flex="1">
            ...
            <Flex align="center" mt={1}>
              <Badge
                colorScheme={
                  statusColorMap[
                    application.status as keyof typeof statusColorMap
                  ]
                }
                variant="subtle"
                mr={2}
          ...
          <Menu>
            ...
            <MenuList>
              ...
              <MenuItem icon={<EditIcon />}>Move to Stage</MenuItem>
    ...
    <Container maxW="full" p={0}>
      ...
      <Box mb={8}>
        ...
        {viewMode === "candidates" ? (
          <Alert status="info" mb={6} borderRadius="lg">
            ...
            <AlertDescription>
              View and manage your talent pool. Focus on candidate profiles,
              skills, and overall candidate database management.
            </AlertDescription>
          </Alert>
        ) : (
          <Alert status="info" mb={6} borderRadius="lg">
            ...
            <AlertDescription>
              Track application workflows and recruitment stages. Manage
              candidates through your hiring process from application to hire.
            </AlertDescription>
          </Alert>
        )}
        ...
        <SimpleGrid columns={{ base: 2, md: 4, lg: 7 }} spacing={4} mb={6}>
          ...
          >
            <StatLabel>
              {viewMode === "candidates" ? "Total Candidates" : "Total"}
            </StatLabel>
      ...
      <Card mb={6} bg={cardBg} borderColor={borderColor}>
        <CardBody>
          ...
          >
                ...
                <Input
                  placeholder={
                    viewMode === "candidates"
                      ...
                      : "Search candidates, positions, emails..."
                  }
                  value={searchTerm}
            ...
            >
              <option value="all">
                {viewMode === "candidates" ? "All Positions" : "All Positions"}
              </option>
              {jobs.map((job) => (
          ...
          >
            ...
            >
              <option value="all">
                {viewMode === "candidates" ? "All Time" : "All Time"}
              </option>
          ...
          <Flex justify="space-between" align="center">
            <Flex align="center" gap={4}>
              <Text fontSize="sm" color="gray.500">
                ...
                {viewMode === "candidates" ? "candidates" : "candidates"}
              </Text>
        ...
        <>
                  ...
                  <Table variant="simple">
                    <Thead>
                      <Tr>
                        ...
                        <Th
                          cursor="pointer"
                          onClick={() => handleSortToggle("candidate_name")}
                        >
                          Candidate{" "}
                        ...
                        <Th
                          cursor="pointer"
                          onClick={() => handleSortToggle("application_date")}
                        >
                          Applied{" "}
                    ...
                    <Tbody>
                      {paginatedApplications.map((application) => (
                        <Tr key={application.id} _hover={{ bg: hoverBg }}>
                          <Td>
                            <Checkbox
                              isChecked={selectedApplications.includes(
                                application.id,
                              )}
                              onChange={(e) =>
                          ...
                          <Td>
                            {application.custom_fields?.years_of_experience ||
                              0}{" "}
                            years
                          </Td>
                          <Td>
          ...
          {displayMode === "pipeline" && viewMode === "applications" && (
            <Grid
              ...
              mb={6}
            >
              {["new", "screening", "interview", "shortlisted"].map(
                (status) => (
                  <Box key={status}>
          ...
          <Flex justify="space-between" align="center" mt={6}>
            <HStack>
              <Text fontSize="sm" color="gray.500">
                ...
                of {filteredApplications.length} candidates
              </Text>
              <Select
                value={pageSize}
      ...
      <Modal isOpen={isBulkActionOpen} onClose={onBulkActionClose}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <Text mb={4}>
              Perform action on {selectedApplications.length} selected
              candidates:
            </Text>
            <FormControl mb={4}>
      ...
      <Modal isOpen={isExportOpen} onClose={onExportClose}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            ...
            <Text fontSize="sm" color="gray.500">
              This will include candidate names, emails, positions, status,
              application dates, experience, and scores.
            </Text>
          </ModalBody>
          <ModalFooter>
