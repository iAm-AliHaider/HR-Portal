...
const defaultCriteria: Record<string, EvaluationCriterion[]> = {
  screening: [
    {
      id: "resumeQuality",
      name: "Resume Quality",
      description: "Overall quality of the resume",
      type: "rating",
      required: true,
    },
    {
      ...
      description: "How well the candidate's experience matches requirements",
      type: "rating",
      required: true,
    },
    {
      id: "skillsMatch",
      name: "Skills Match",
      description: "Relevance of candidate's skills to the role",
      type: "rating",
      required: true,
    },
    {
      ...
      description: "Relevance of candidate's education to the role",
      type: "rating",
      required: true,
    },
    {
      ...
      description: "Overall first impression of the candidate",
      type: "rating",
      required: true,
    },
    {
      id: "meetsCoreRequirements",
      name: "Meets Core Requirements",
      description: "Does the candidate meet the core requirements?",
      type: "yesno",
      required: true,
    },
  ],
  interview: [
    ...
    },
    {
      ...
      description: "Depth and relevance of past experience",
      type: "rating",
      required: true,
    },
    {
      ...
      description: "Ability to think through complex problems",
      type: "rating",
      required: true,
    },
    {
      id: "teamwork",
      name: "Teamwork",
      description: "Experience working in teams and collaboration skills",
      type: "rating",
      required: true,
    },
    {
  ...
  technical: [
    ...
    },
    {
      id: "codingSkills",
      name: "Coding Skills",
      description: "Quality and efficiency of code",
      type: "rating",
      required: true,
    },
    {
    ...
    },
    {
      ...
      description: "Methodology to solving technical problems",
      type: "rating",
      required: true,
    },
    {
    ...
    },
    {
      id: "codeQuality",
      name: "Code Quality",
      description: "Attention to best practices and clean code",
      type: "rating",
      required: true,
    },
  ],
  final: [
    {
      ...
      description: "Final impression of the candidate",
      type: "rating",
      required: true,
    },
    {
      id: "strengths",
      name: "Key Strengths",
      description: "Candidate's primary strengths",
      type: "text",
      required: true,
    },
    {
      ...
      description: "Candidate's areas for improvement",
      type: "text",
      required: true,
    },
    {
      id: "teamFit",
      name: "Team Fit",
      description: "How well the candidate would fit with the team",
      type: "rating",
      required: true,
    },
    {
      ...
      description: "Long-term potential of the candidate",
      type: "rating",
      required: true,
    },
    {
...
export default function CandidateEvaluation({
  ...
  const [criteria, setCriteria] = useState<EvaluationCriterion[]>(
    defaultCriteria[stage] || [],
  );
  const [overallNotes, setOverallNotes] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [decisionMade, setDecisionMade] = useState(false);
  const [decision, setDecision] = useState<"advance" | "reject" | "hold">(
    "hold",
  );
  ...
  useEffect(() => {
    if (existingEvaluation) {
      if (
        ...
        Array.isArray(existingEvaluation.criteria)
      ) {
  ...
  const calculateOverallScore = () => {
    const ratingCriteria = criteria.filter(
      (c) => c.type === "rating" && c.score !== undefined,
    );
    if (ratingCriteria.length === 0) return 0;

    const sum = ratingCriteria.reduce(
      (total, criterion) => total + (criterion.score || 0),
      0,
    );
  ...
  const validateForm = () => {
    ...
    if (missingFields.length > 0) {
      toast.error(
        `Please complete all required fields: ${missingFields.map((f) => f.name).join(", ")}`,
      );
      return false;
    }
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4">
          <div>
            ...
            <p className="text-gray-500">
              {position} - {stage.charAt(0).toUpperCase() + stage.slice(1)}{" "}
              Evaluation
            </p>
          </div>
          <div className="mt-2 md:mt-0">
            <div className="text-lg font-semibold flex items-center">
              ...
              <span
                className={`ml-2 ${getScoreColor(calculateOverallScore())}`}
              >
                ...
                <div className="flex justify-between items-start mb-2">
                  ...
                  {criterion.type === "rating" &&
                    criterion.score !== undefined && (
                      <span
                        className={`font-bold ${getScoreColor(criterion.score)}`}
                      >
                        {criterion.score} / 5
                      </span>
                    )}
                ...
                {criterion.type === "rating" && (
                  <div className="mt-4">
                    ...
                    <Slider
                      ...
                      step={0.5}
                      onValueChange={(value) =>
                        updateCriterion(criterion.id, "score", value[0])
                      }
                    />
                  </div>
                )}
                ...
                {criterion.type === "yesno" && (
                  <div className="mt-4">
                    <RadioGroup
                      ...
                      onValueChange={(value) =>
                        updateCriterion(criterion.id, "answer", value === "yes")
                      }
                    >
                ...
                {criterion.type === "text" && (
                  <div className="mt-4">
                    <Textarea
                      ...
                      onChange={(e) =>
                        updateCriterion(criterion.id, "answer", e.target.value)
                      }
            ...
            <div className="mt-6">
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div
                    className={`p-4 border rounded-lg cursor-pointer ${decision === "advance" ? "bg-green-50 border-green-300" : "hover:bg-gray-50"}`}
                  >
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="advance" id="decision-advance" />
                      <Label htmlFor="decision-advance" className="font-medium">
                        Advance Candidate
                      </Label>
                    </div>
                  ...
                  <div
                    className={`p-4 border rounded-lg cursor-pointer ${decision === "hold" ? "bg-yellow-50 border-yellow-300" : "hover:bg-gray-50"}`}
                  >
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="hold" id="decision-hold" />
                      <Label htmlFor="decision-hold" className="font-medium">
                        Hold for Review
                      </Label>
                    </div>
                    <p className="text-sm text-gray-500 mt-2">
                      Needs additional consideration or review from other team
                      members
                    </p>
                  </div>

                  <div
                    className={`p-4 border rounded-lg cursor-pointer ${decision === "reject" ? "bg-red-50 border-red-300" : "hover:bg-gray-50"}`}
                  >
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="reject" id="decision-reject" />
                      <Label htmlFor="decision-reject" className="font-medium">
                        Reject Candidate
                      </Label>
                    </div>
                    <p className="text-sm text-gray-500 mt-2">
