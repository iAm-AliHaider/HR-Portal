...
const LeaveRequestForm: React.FC = () => {
  ...
  const [leaveBalances] = useState<LeaveBalance[]>([
    ...
    { leaveType: 'maternity', allocated: 120, used: 0, pending: 0, remaining: 120 },
    { leaveType: 'paternity', allocated: 14, used: 0, pending: 0, remaining: 14 }
  ]);
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 1) {
      if (!request.leaveType) newErrors.leaveType = 'Please select a leave type';
      ...
      if (request.leaveType && request.startDate) {
        const selectedLeaveType = leaveTypes.find(type => type.id === request.leaveType);
      ...
      if (request.leaveType && request.totalDays > 0) {
        const balance = leaveBalances.find(b => b.leaveType === request.leaveType);
    ...
    if (step === 2) {
      ...
      if (request.reason.trim().length < 10) newErrors.reason = 'Please provide a detailed reason (minimum 10 characters)';
    }
  ...
  const getLeaveTypeById = (id: string) => leaveTypes.find(type => type.id === id);
  const getBalanceByType = (type: string) => leaveBalances.find(b => b.leaveType === type);
          ...
          {leaveTypes.map((type) => {
            ...
            return (
              <div
                ...
                onClick={() => setRequest(prev => ({ ...prev, leaveType: type.id }))}
              >
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => setRequest(prev => ({ ...prev, emergencyContact: e.target.value }))}
          className="w-full p-3 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
      ...
      <div>
        ...
        <textarea
          ...
          onChange={(e) => setRequest(prev => ({ ...prev, handoverNotes: e.target.value }))}
          rows={3}
          className="w-full p-3 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex items-center mt-1">
                  <span className="text-lg mr-2">{selectedLeaveType?.icon}</span>
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <ul className="text-sm text-blue-700 mt-1 space-y-1">
                <li>• Your remaining balance after this request: {(balance?.remaining || 0) - request.totalDays} days</li>
    ...
    <div className="text-center space-y-6">
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        <p className="text-sm text-green-700">
          ...
          1. Your manager will be notified to review your request<br />
          2. You'll receive an email notification once it's approved or rejected<br />
