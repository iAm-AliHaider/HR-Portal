...
const companyBenefits = [
  ...
  },
  {
    ...
    description: "Dental coverage for preventive care",
  },
  {
    id: "vision",
    name: "Vision Insurance",
    description: "Vision coverage for eye care",
  },
  {
    id: "401k",
    name: "401(k) Plan",
  ...
  },
  {
    id: "pto",
    name: "Paid Time Off",
    description: "15 days of vacation + holidays",
  },
  {
    id: "parental",
    name: "Parental Leave",
    description: "12 weeks of paid parental leave",
  },
  {
    id: "remote",
    name: "Remote Work",
  ...
  },
  {
    ...
    description: "$1,000 annual education budget",
  },
  {
    id: "wellness",
    name: "Wellness Program",
    description: "Gym reimbursement and wellness activities",
  },
];
...
export default function OfferManagement({
  ...
  const [sendReminderBeforeExpiration, setSendReminderBeforeExpiration] =
    useState(true);
  ...
  useEffect(() => {
    if (existingOffer) {
      ...
      if (existingOffer.contractType)
        setContractType(existingOffer.contractType);
      ...
      if (existingOffer.offerExpiration)
        setOfferExpiration(existingOffer.offerExpiration);
      if (existingOffer.additionalTerms)
        setAdditionalTerms(existingOffer.additionalTerms);
      if (existingOffer.personalMessage)
        setPersonalMessage(existingOffer.personalMessage);
      ...
      if (existingOffer.sendReminderBeforeExpiration !== undefined) {
        setSendReminderBeforeExpiration(
          existingOffer.sendReminderBeforeExpiration,
        );
      }
    } else {
  ...
  const handleSave = async (status: "draft" | "sent") => {
    ...
    try {
      const offerData = {
        ...
        offerExpiration,
        benefits: companyBenefits.filter((benefit) =>
      ...
      toast.success(
        status === "sent" ? "Offer sent successfully" : "Offer saved as draft",
      );
    } catch (error) {
      toast.error(
        status === "sent" ? "Failed to send offer" : "Failed to save offer",
      );
  ...
  const updateOfferStatus = async (
    newStatus: "accepted" | "rejected" | "negotiating",
  ) => {
    setIsSubmitting(true);
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-md shadow-sm border p-4">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center">
          ...
          {offerStatus !== "draft" && (
            <div
              className={`mt-2 md:mt-0 px-4 py-2 rounded-md border text-sm font-medium ${
                offerStatus === "sent"
          ...
          <CardContent>
            <div className="bg-gray-50 p-4 rounded-md mb-6">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Last Updated:{" "}
                {new Date(existingOffer.updatedAt).toLocaleString()}
              </p>
            </div>

            <RadioGroup
              value={offerStatus}
              onValueChange={(value: any) => setOfferStatus(value)}
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div
                  className={`p-4 border rounded-lg cursor-pointer ${offerStatus === "accepted" ? "bg-green-50 border-green-300" : "hover:bg-gray-50"}`}
                >
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="accepted" id="status-accepted" />
                    <Label htmlFor="status-accepted" className="font-medium">
                      Accepted
                    </Label>
                  </div>
                ...
                <div
                  className={`p-4 border rounded-lg cursor-pointer ${offerStatus === "negotiating" ? "bg-yellow-50 border-yellow-300" : "hover:bg-gray-50"}`}
                >
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem
                      value="negotiating"
                    ...
                    />
                    <Label htmlFor="status-negotiating" className="font-medium">
                      Negotiating
                    </Label>
                  </div>
                  <p className="text-sm text-gray-500 mt-2">
                ...
                <div
                  className={`p-4 border rounded-lg cursor-pointer ${offerStatus === "rejected" ? "bg-red-50 border-red-300" : "hover:bg-gray-50"}`}
                >
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="rejected" id="status-rejected" />
                    <Label htmlFor="status-rejected" className="font-medium">
                      Rejected
                    </Label>
                  </div>
                  <p className="text-sm text-gray-500 mt-2">
            ...
            <div className="mt-6 text-right">
              <Button
                ...
                onClick={() =>
                  updateOfferStatus(
                    offerStatus as "accepted" | "rejected" | "negotiating",
                  )
                }
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <Label htmlFor="offer-expiration">
                ...
                <div>
                  ...
                  <div className="relative mt-1">
                    ...
                    <Input
                      ...
                      value={baseSalary}
                      onChange={(e) =>
                        setBaseSalary(formatCurrency(e.target.value))
                      }
                    />
                  </div>
                </div>
                ...
                <div>
                  <Label htmlFor="sign-on-bonus">
                    Sign-On Bonus (USD, Optional)
                  </Label>
                  <div className="relative mt-1">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      <span className="text-gray-500">$</span>
                    ...
                    <Input
                      ...
                      value={signOnBonus}
                      onChange={(e) =>
                        setSignOnBonus(formatCurrency(e.target.value))
                      }
                    />
                  </div>
                </div>
                ...
                <div className="border rounded-md p-6 bg-white">
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-gray-800">
                      ...
                      <strong>{jobPosition}</strong> at our company. This is a{" "}
                      <strong>
                        {contractTypes
                          .find((t) => t.value === contractType)
                          ?.label.toLowerCase()}
                      </strong>{" "}
                      position, reporting to the {department} department.
                    </p>
                    ...
                    <div className="space-y-2">
                      ...
                      <ul className="list-disc list-inside text-gray-800 ml-2 space-y-1">
                        <li>
                          Annual Base Salary: <strong>${baseSalary}</strong>
                        </li>
                        {signOnBonus && (
                          <li>
                            Sign-On Bonus: <strong>${signOnBonus}</strong>
                          </li>
                        )}
                      </ul>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <ul className="list-disc list-inside text-gray-800 ml-2 space-y-1">
                        {selectedBenefits.map((id) => {
                          const benefit = companyBenefits.find(
                            (b) => b.id === id,
                          );
                          return benefit ? (
                            <li key={id}>
                    ...
                    <p className="text-gray-800">
                      ...
                      <strong>
                        {new Date(offerExpiration).toLocaleDateString()}
                      </strong>
                      .
                    </p>

                    <p className="text-gray-800">
                      We are excited about the possibility of you joining our
                      team and look forward to your response.
                    </p>
