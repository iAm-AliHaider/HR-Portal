...
const OfferForm = ({ offerId, applicationId, onSave }: OfferFormProps) => {
        ...
        if (offerId) {
          ...
          if (offerData) {
            ...
            if (offerData.application_id) {
              const appData = await getApplicationById(
  ...
  const validateForm = (): boolean => {
    ...
    if (!offer.salary?.amount)
      newErrors["salary.amount"] = "Salary amount is required";
    ...
    if (
      ...
      newErrors["expiration_date"] =
        "Expiration date must be before start date";
    }
  ...
  const handleSubmit = async (
    e: React.FormEvent,
    submitAction: "save_draft" | "submit_for_approval" = "save_draft",
  ) => {
    e.preventDefault();
            ...
            <GridItem>
              <FormControl isRequired isInvalid={!!errors["salary.amount"]}>
                ...
                <NumberInput
                  ...
                  onChange={(_, value) =>
                    handleNumberChange("salary.amount", value)
                  }
                >
                  <NumberInputField />
                  <NumberInputStepper>
                ...
                <Flex gap={4} flexWrap="wrap">
                  <Box flexGrow={1}>
                    ...
                    <NumberInput
                      ...
                      onChange={(_, value) =>
                        handleNumberChange("bonus.amount", value)
                      }
                    >
                      <NumberInputField />
                    </NumberInput>
            ...
            <GridItem colSpan={{ base: 1, md: 3 }}>
              <FormControl>
                ...
                <Input
                  ...
                  value={
                    offer.equity
                      ? `${offer.equity.amount} ${offer.equity.type}`
                      : ""
                  }
                  onChange={(e) => {
                    ...
                    setOffer((prev) => ({
                      ...prev,
                      equity_description: e.target.value,
                    }));
                  }}
