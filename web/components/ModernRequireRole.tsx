...
export function ModernRequireRole({ 
  ...
  useEffect(() => {
    ...
    if (isDevelopment) {
      ...
      if (storedEmail) {
        const matchedAccount = mockAccounts.find(account => account.email === storedEmail);
        if (matchedAccount) {
  ...
  useEffect(() => {
    if (isDevelopment && user?.email) {
      const matchedAccount = mockAccounts.find(account => account.email === user.email);
      if (matchedAccount) {
  ...
  if (error) {
    ...
    if (showInlineError) {
      return <Card><CardContent className="p-8">{errorContent}</CardContent></Card>;
    }
  ...
  const hasPermission = allowed.includes(effectiveRole ?? '') || (isDevelopment && mockRole);
  ...
  if (!hasPermission) {
      ...
      <div className="text-center">
        ...
        <div className="bg-gray-50 rounded-lg p-4 mb-6 text-left">
          <div className="text-sm space-y-1">
            <p><span className="font-medium">Current role:</span> {effectiveRole || 'none'}</p>
            <p><span className="font-medium">Required roles:</span> {allowed.join(', ')}</p>
          </div>
        </div>
    ...
    if (showInlineError) {
      return <Card><CardContent className="p-8">{accessDeniedContent}</CardContent></Card>;
    }
