...
export function getProductionConfig(): ProductionConfig {
  ...
  return {
    ...
    features: {
      ...
      enableFallbackAuth: process.env.NEXT_PUBLIC_ENABLE_FALLBACK_AUTH === 'true',
    },
    security: {
      maxFileSize: parseInt(process.env.NEXT_PUBLIC_MAX_FILE_SIZE || '10485760'),
      allowedFileTypes: (process.env.NEXT_PUBLIC_ALLOWED_FILE_TYPES || '.pdf,.doc,.docx,.xls,.xlsx,.csv,.jpg,.jpeg,.png').split(','),
      rateLimitRequests: parseInt(process.env.NEXT_PUBLIC_RATE_LIMIT_REQUESTS || '100'),
      rateLimitWindow: parseInt(process.env.NEXT_PUBLIC_RATE_LIMIT_WINDOW || '900000'),
    },
    email: {
      supportEmail: process.env.NEXT_PUBLIC_SUPPORT_EMAIL || 'support@yourcompany.com',
      fromEmail: process.env.NEXT_PUBLIC_FROM_EMAIL || 'noreply@yourcompany.com',
    },
  };
}
...
export function validateProductionReadiness(): { ready: boolean; issues: string[] } {
  ...
  try {
    ...
    if (config.features.enableDebugRoutes) {
      issues.push('Debug routes are enabled (should be disabled in production)');
    }

    if (config.features.enableFallbackAuth) {
      issues.push('Fallback authentication is enabled (should be disabled in production)');
    }
