...
function fixLearningPortalPage() {
  ...
  const getStatusColor = (status: string) => {
    ...
    if (newContent !== content) {
      ...
      filesChanged++;
      logChange('pages/learning.tsx', 'Enhanced API error handling with timeout protection and fallback data');
    }
...
function fixEmployeeLearningPortal() {
  const filePath = path.join(process.cwd(), 'pages/employee/learning-portal.tsx');
  ...
  const recommendedCourses = [`
    ...
    newContent = newContent.replace(
      ...
      (match, offset) => {
        ...
        if (currentLine.includes('Project Management')) return `thumbnail: '📋'`;
        if (currentLine.includes('Business Analytics')) return `thumbnail: '📈'`;
    ...
    if (newContent !== content) {
      fs.writeFileSync(filePath, newContent, 'utf8');
      filesChanged++;
      logChange('pages/employee/learning-portal.tsx', 'Replaced broken image paths with emoji thumbnails and added error handling');
    }
...
function createCourseImagePlaceholders() {
  try {
    ...
    filesChanged++;
    logChange('public/course-images/', 'Created placeholder image files to prevent 404 errors');
...
function enhanceGoTrueClientSuppression() {
  ...
  webpack: (config) => {
    ...
    if (newContent !== content) {
      fs.writeFileSync(filePath, newContent, 'utf8');
      filesChanged++;
      logChange('next.config.js', 'Enhanced GoTrueClient warning suppression with webpack configuration');
    }
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!fs.existsSync(filePath)) {
      ...
      filesChanged++;
      logChange('pages/api/learning/courses.ts', 'Created Learning API endpoint with mock data');
    }
