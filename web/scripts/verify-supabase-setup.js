  ...
  for (const table of tables) {
    try {
      ...
      if (count < table.minCount) {
        logWarning(
          `${table.name} has insufficient data: ${count} rows (expected at least ${table.minCount})`,
        );
        missingData.push(table.name);
      } else {
...
async function testAuthentication() {
  ...
  try {
    ...
    logSuccess(
      `Profile found for the test account: ${profile.name} (${profile.role})`,
    );
    return true;
  } catch (error) {
...
async function main() {
  ...
  if (missingTables.length === 0) {
    ...
    logError(
      `${missingTables.length} tables are missing: ${missingTables.join(", ")}`,
    );
  }
  ...
  if (existingTables.length > 0) {
    ...
    if (missingData.length === 0) {
      ...
      logWarning(
        `${missingData.length} tables are missing data: ${missingData.join(", ")}`,
      );
    }
  }
  ...
  if (missingTables.length > 0) {
    console.log(
      chalk.yellow("1. Run the migration script to create missing tables"),
    );
  }
  if (!authSuccess) {
    console.log(
      chalk.yellow("2. Check authentication setup and test accounts"),
    );
  }
