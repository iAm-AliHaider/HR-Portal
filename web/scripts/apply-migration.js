...
async function applySqlMigration(filePath) {
  try {
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      const { error } = await supabase.rpc('pgbouncer_exec_sql', { sql: statement + ';' });
...
async function main() {
  ...
  const migrationFile = path.resolve(__dirname, '../supabase/migrations/20250528000000_complete_requirements.sql');
  ...
  if (missingTables.length === 0) {
    ...
    logError(`${missingTables.length} tables are still missing: ${missingTables.join(', ')}`);
