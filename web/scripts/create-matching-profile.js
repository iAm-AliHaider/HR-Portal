  ...
  try {
    ...
    if (createError) {
      if (
        ...
        console.log(
          "⚠️ Email conflict detected, creating with temporary email...",
        );
        ...
        if (tempCreateError) {
          console.error(
            "❌ Failed to create profile with temp email:",
            tempCreateError.message,
          );
          return;
        }
        ...
        if (updateError) {
          console.log(
            "⚠️ Could not update to original email, use temp email for login",
          );
        } else {
    ...
    console.log(
      `3. Log in with: ${verifyProfile ? verifyProfile.email : originalEmail} / HRPortal2024!`,
    );
