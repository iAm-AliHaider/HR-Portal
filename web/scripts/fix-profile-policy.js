...
async function fixProfileRLSPolicies() {
  ...
  try {
    ...
    const { error: dropError } = await supabase.rpc('pgbouncer_exec_sql', { sql: sqlDropPolicies });
    ...
    const { error: createError } = await supabase.rpc('pgbouncer_exec_sql', { sql: sqlCreatePolicies });
    ...
    const { error: updateError } = await supabase.rpc('pgbouncer_exec_sql', { sql: updateRequestTypesSchema });
    
    if (updateError) {
      console.error('Error updating request_types schema:', updateError.message);
    } else {
