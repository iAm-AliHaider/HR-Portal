...
async function createTestAccount(account) {
  ...
  try {
    ...
    if (account.userData.role === 'employee') {
      ...
      if (employeeError) {
        console.warn(`⚠️ Failed to create employee record for ${account.email}:`, employeeError.message);
      } else {
    ...
    console.error(`❌ Unexpected error creating account for ${account.email}:`, error.message);
    return false;
  }
}
...
async function setupTestAccounts() {
  ...
  console.log(`\n📊 Setup Results:`);
  console.log(`✅ Successfully created: ${successCount}/${totalAccounts} accounts`);
  
  if (successCount === totalAccounts) {
    console.log('\n🎉 All test accounts created successfully!');
    console.log('\n📝 Test Account Credentials:');
    TEST_ACCOUNTS.forEach(account => {
      console.log(`   ${account.userData.role.toUpperCase()}: ${account.email} / ${account.password}`);
    });
...
if (require.main === module) {
  ...
  if (!SUPABASE_SERVICE_KEY || SUPABASE_SERVICE_KEY.includes('YOUR_SERVICE_ROLE_KEY_HERE')) {
    showInstructions();
