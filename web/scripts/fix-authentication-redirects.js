...
function removeRequireRoleWrapper(content) {
  ...
  content = content.replace(/import\s*{\s*RequireRole\s*}\s*from\s*['"][^'"]+['"];\s*\n/g, '');
  ...
  content = content.replace(requireRolePattern, (match) => {
    ...
    const innerContent = match.replace(/<RequireRole[^>]*>/, '').replace(/<\/RequireRole>$/, '');
    return innerContent;
  });
...
function processFile(filePath) {
  try {
    ...
    if (content.includes('ModernRequireRole') && !content.includes('from \'@/components/RequireRole\'') && !content.includes('from \'../components/RequireRole\'')) {
      return;
    }
    ...
    if (filePath.includes('/jobs/') || filePath.includes('jobs.tsx') || filePath.includes('candidate.')) {
    ...
    // For other pages, replace with ModernRequireRole with fallbackToPublic
      ...
      if (newContent !== content) {
        changed = true;
        logChange(filePath, 'Replaced RequireRole with ModernRequireRole (fallbackToPublic)');
      }
    }
...
fs.writeFileSync('authentication-redirect-fixes.json', JSON.stringify(report, null, 2));
...
console.log('4. Check that protected pages still have appropriate access control'); 