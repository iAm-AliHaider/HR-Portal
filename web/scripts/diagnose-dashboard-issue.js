...
criticalFiles.forEach((file) => {
  ...
  console.log(
    `   ${check.exists ? "✅" : "❌"} ${file} ${check.exists ? `(${check.size} bytes)` : "(missing)"}`,
  );
  if (check.error) {
...
if (!dashboardCheck.valid) {
  console.log(
    `   Issues: ${dashboardCheck.issues?.join(", ") || dashboardCheck.error}`,
  );
}
...
if (!layoutCheck.valid) {
  console.log(
    `   Issues: ${layoutCheck.issues?.join(", ") || layoutCheck.error}`,
  );
}
...
buildFiles.forEach((file) => {
  const check = checkFile(file);
  console.log(
    `   ${check.exists ? "✅" : "❌"} ${file} ${check.exists ? `(${check.size} bytes)` : "(not built)"}`,
  );
});
